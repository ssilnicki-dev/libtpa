...
static inline int shell_pack_message(struct shell_buf *cmd, const char *name, int argc, char *argv[]) {
    int i;
    int n;
    int len = 0;
...
static inline int shell_client_exec(int fd, const char *name, int argc, char *argv[]) {
    ...
    if (cmd.hdr.len > sizeof(struct shell_buf_hdr)) printf("%s", cmd.data);
