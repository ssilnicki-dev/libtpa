...
static void iov_buf_free(void *addr, void *param) {
    ...
    if (--pkt->nr_read_seg == 0) {
        ...
        if (unlikely(pkt->flags & PKT_FLAG_MEASURE_READ_LATENCY)) tsock_read_latency_update(tsock, pkt);

        tsock->worker->nr_in_process_mbuf -= pkt->mbuf.nb_segs;

        debug_assert(pkt->mbuf.pool == generic_pkt_pool->pool[0] || pkt->mbuf.pool == generic_pkt_pool->pool[1]);
        packet_free(pkt);
    }
}

static inline size_t pkt_to_iov_one_seg(struct tpa_iovec *iov, struct packet *head, struct packet *pkt) {
    ...
    if (TCP_SEG(head)->len == 0 && (head->flags & PKT_FLAG_MEASURE_READ_LATENCY)) head->read_tsc.drain = rte_rdtsc();
    ...
    do {                                                                                                                                   \
        ...
        if (unlikely(tsock->err != 0)) {                                                                                                   \
            ...
            return -1;                                                                                                                     \
                                                                                                                                           ...
                                                                                                                                           \
        if (unlikely(tsock->state == TCP_STATE_LISTEN) || tsock->state == TCP_STATE_CLOSED) {                                              \
            errno = ENOTCONN;                                                                                                              \
            return -1;                                                                                                                     \
        }                                                                                                                                  \
    } while (0)
...
ssize_t tsock_zreadv(struct tcp_sock *tsock, struct tpa_iovec *iov, int nr_iov) {
    ...
    if (ctx.size == 0) {
        if (tsock->flags & TSOCK_FLAG_EOF) return 0;
    ...
    if (unlikely(trace_cfg.more_trace)) trace_tcp_zreadv(tsock, ctx.size, nr_iov, ctx.idx, tcp_rxq_readable_count(&tsock->rxq));
...
static inline int do_parse_tcp_packet(struct packet *pkt) {
    ...
    if (unlikely(pkt->mbuf.pkt_len < pkt->hdr_len)) return -ERR_PKT_INVALID_LEN;
...
int parse_tcp_opts(struct tcp_opts *opts, struct packet *pkt) {
    ...
    while (off < len) {
        ...
        if (opt->type == TCP_OPT_EOL_KIND) return 0;
        ...
        if (opt->len < 2) return -ERR_INVALID_TCP_OPT_LEN;

        off += opt->len;
        if (off > len) return -ERR_INVALID_TCP_OPT_LEN;
        ...
        case TCP_OPT_WSCALE_KIND:
            ...
            if (opts->wscale > TCP_WSCALE_MAX) opts->wscale = TCP_WSCALE_MAX;
            break;
        ...
        case TCP_OPT_SACK_KIND:
            ...
            if (opt->len > TCP_OPT_SACK_LEN(TCP_MAX_NR_SACK_BLOCK)) return -ERR_INVALID_TCP_OPT_LEN;

            if ((opt->len - 2) % sizeof(struct tcp_sack_block) != 0) return -ERR_INVALID_TCP_OPT_LEN;
...
#define TS_OPT_HDR (TCP_OPT_NOP_KIND << 24 | TCP_OPT_NOP_KIND << 16 | TCP_OPT_TS_KIND << 8 | TCP_OPT_TS_LEN << 0)
...
static inline int tcp_rcv_enqueue(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    if (unlikely(TCP_SEG(pkt)->len > tsock->rcv_wnd)) tcp_packet_cut(pkt, TCP_SEG(pkt)->len - tsock->rcv_wnd, CUT_TAIL);

    if (unlikely(TCP_SEG(pkt)->len == 0)) return 0;

    if (unlikely(tcp_rxq_enqueue_burst(&tsock->rxq, (void **)&pkt, 1) != 1)) return -ERR_TCP_RXQ_ENQUEUE_FAIL;
    ...
    if (unlikely(pkt->flags & PKT_FLAG_MEASURE_READ_LATENCY)) pkt->read_tsc.submit = rte_rdtsc();

    tsock_update_last_ts(tsock, LAST_TS_RCV_DATA);
    trace_tcp_rcv_enqueue(tsock, tsock->rcv_nxt, TCP_SEG(pkt)->len, tsock->rcv_wnd, tcp_rxq_readable_count(&tsock->rxq));

    return 0;
}

static int sack_merge(struct tcp_sack_block *blk, uint32_t start, uint32_t end) {
    if (seq_lt(end, blk->start) || seq_gt(start, blk->end)) return 0;

    if (seq_lt(start, blk->start)) blk->start = start;
    if (seq_gt(end, blk->end)) blk->end = end;
...
static void sack_try_merge_blocks(struct tcp_sock *tsock) {
    ...
    for (i = 1; i < tsock->nr_sack_block; i++) {
        if (!sack_merge(blk, tsock->sack_blocks[i].start, tsock->sack_blocks[i].end)) tsock->sack_blocks[nr_sack++] = tsock->sack_blocks[i];
    }

    tsock->nr_sack_block = nr_sack;
}

#define SACK_SWAP(a, b)                                                                                                                    \
    do {                                                                                                                                   \
...
static void sack_update(struct tcp_sock *tsock, uint32_t start, uint32_t len) {
    ...
    memmove(&tsock->sack_blocks[1], &tsock->sack_blocks[0], nr_to_shift * sizeof(struct tcp_sack_block));
...
static void sack_regulate(struct tcp_sock *tsock) {
    ...
    for (i = 0; i < tsock->nr_sack_block; i++) {
        blk = &tsock->sack_blocks[i];
        if (seq_gt(blk->start, tsock->rcv_nxt)) tsock->sack_blocks[nr_sack++] = *blk;
    }
...
void tsock_remove_ooo_pkt(struct tcp_sock *tsock, struct packet *pkt) {
    ...
    if (tsock->last_ooo_pkt == pkt) tsock->last_ooo_pkt = NULL;
}
...
static void ooo_queue_drain(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    while (pkt) {
        ...
        if (TCP_SEG(pkt)->seq != tsock->rcv_nxt) break;
    ...
    if (tsock->nr_ooo_pkt == 0) {
        ...
        tsock_trace_archive(tsock->trace, "ooo-%.3fms", (double)recover_time / 1e3);
...
static int ooo_queue_insert(struct tcp_sock *tsock, struct packet *prev, struct packet *pkt) {
    ...
    if (TCP_SEG(pkt)->len == 0) return -ERR_TCP_RCV_OOO_DUP;
...
static int tcp_rcv_data_ooo(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    ...
    if (tsock->nr_ooo_pkt == 0) tsock->ooo_start_ts = worker->ts_us;
    ...
    TAILQ_FOREACH(next, &tsock->rcv_ooo_queue, node) {
        ...
        if (seq_gt(TCP_SEG(next)->seq, TCP_SEG(pkt)->seq)) break;
        prev = next;
    }

    if (prev) {
        if (TCP_SEG(pkt)->seq == TCP_SEG(prev)->seq && TCP_SEG(pkt)->len > TCP_SEG(prev)->len) {
...
static inline void tsock_set_ack_flag(struct tcp_sock *tsock, int ack_now_flag) {
     ...
     * second segment.
     */
    if (tcp_cfg.delayed_ack == 0 || tsock->rcv_nxt - tsock->last_ack_sent >= tsock->snd_mss) ack_now_flag = TSOCK_FLAG_ACK_NOW;

    tsock->flags |= TSOCK_FLAG_ACK_NEEDED | ack_now_flag;
}

static inline int tcp_rcv_data(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    uint32_t to_cut;
    int err;

    if (unlikely(TCP_SEG(pkt)->len == 0)) {
        if (has_flag_fin(pkt) && TCP_SEG(pkt)->seq == tsock->rcv_nxt) tsock->rcv_nxt += 1;
    ...
    if (TCP_SEG(pkt)->seq == tsock->rcv_nxt) {
        ...
        if (ooo_head && seq_ge(TCP_SEG(pkt)->seq + TCP_SEG(pkt)->len, TCP_SEG(ooo_head)->seq)) goto ooo_rcv;
...
static inline int ack_sent_data(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *ack_pkt, uint32_t acked_len,
                                uint32_t *rtt) {
    ...
    *rtt = 0;
    do {
        ...
        tsock_trace_ack_sent_data(tsock, desc, acked_len, nr_acked_pkt, worker->ts_us);
        ...
        if (unlikely(desc->flags & TX_DESC_FLAG_SACKED)) tsock->sacked_bytes -= desc->len;

        /* rtt for retransmit pkts is excluded */
        if (*rtt == 0 && (desc->flags & TX_DESC_FLAG_RETRANS) == 0) *rtt = worker->ts_us - desc->ts_us;

        if (unlikely(desc->flags & TX_DESC_FLAG_MEASURE_LATENCY)) {
            if (!now) now = rte_rdtsc();
    ...
    if ((uint16_t)(txq->nxt - txq->una) < nr_acked_pkt) txq->nxt = txq->una + nr_acked_pkt;
    ...
    if (tcp_txq_free_count(txq)) tsock_event_add(tsock, TPA_EVENT_OUT);
...
static inline void update_cwnd(struct tpa_worker *worker, struct tcp_sock *tsock, uint32_t acked) {
    ...
    if (worker->ts_us - tsock->snd_cwnd_ts_us < (tsock->srtt >> 3)) return;

    /* meaning it's not cwnd limited */
    if (tsock->snd_cwnd_uncommited < tsock->snd_cwnd) goto out;
...
static inline void handle_fast_retransmit(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt, uint32_t acked) {
    int retrans_budget = tsock->snd_mss;
    uint32_t sacked_bytes;
    uint32_t cwnd;

    if (likely(tsock->nr_dupack < 3 && tsock->retrans_stage != FAST_RETRANS)) return;
    ...
    if (tsock->retrans_stage == NONE && seq_gt(TCP_SEG(pkt)->ack, tsock->snd_recover)) {
        ...
        if (tsock->ts_ok) tsock->retrans.ts_val = us_to_tcp_ts(worker->ts_us);
    }

    if (tsock->retrans_stage != FAST_RETRANS) return;
    ...
    if (seq_lt(TCP_SEG(pkt)->ack, tsock->snd_recover)) {
        ...
        if (tsock->sack_ok) retrans_budget = RTE_MIN(tsock->sacked_bytes, tsock->snd_ssthresh);
...
static int sort_sack(int nr_sack, struct tcp_sack_block *blocks) {
    ...
    for (i = 1; i < nr_sack; i++) {
        if (seq_lt(blocks[i].start, prev_end)) return -ERR_TCP_SACK_INTERSECT;
...
static void mark_desc_sacked(struct tpa_worker *worker, struct tcp_sock *tsock, struct tx_desc *desc) {
    if (desc->flags & TX_DESC_FLAG_SACKED) return;

    tsock->sacked_bytes += desc->len;
    desc->flags |= TX_DESC_FLAG_SACKED;
    trace_tcp_desc_sacked(tsock, desc->seq, desc->len, worker->ts_us - desc->ts_us, desc->flags);
}

static __rte_noinline void tcp_rcv_sack(struct tpa_worker *worker, struct tcp_sock *tsock, struct tcp_opts *opts) {
    struct tcp_txq *txq = &tsock->txq;
    ...
    for (i = 0; i < nr_sack; i++) {
        ...
        if (seq_gt(blk->end, tsock->snd_nxt)) continue;

        while (1) {
            desc = tcp_txq_peek_for_write(txq, txq->una, desc_off++);
            if (!desc) break;

            if (seq_ge(desc->seq + desc->len, tsock->snd_nxt)) break;

            if (seq_ge(desc->seq + desc->len, blk->end)) break;

            if (seq_ge(desc->seq, blk->start)) mark_desc_sacked(worker, tsock, desc);
        }
    }
}

static inline void tsock_established(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt);
static inline int tcp_rcv_ack(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt, struct tcp_opts *opts) {
    int acked_len;
    int err;

    if (!has_flag_ack(pkt)) return -ERR_TCP_NO_ACK;

    if (TCP_SEG(pkt)->flags == TCP_FLAG_ACK && TCP_SEG(pkt)->len == 0) WORKER_TSOCK_STATS_INC(worker, tsock, PURE_ACK_IN);
    ...
    case TCP_STATE_TIME_WAIT:
        ...
        if (TCP_SEG(pkt)->ack == tsock->snd_una) {
            ...
            if (TCP_SEG(pkt)->len == 0 && TCP_SEG(pkt)->wnd == (tsock->snd_wnd >> tsock->snd_wscale) && tsock->snd_nxt != tsock->snd_una) {
                tsock->nr_dupack += 1;
        ...
        /* don't count on SYN */
        if (unlikely(tsock->closed_at_syn_rcvd)) acked_len -= 1;

        if (unlikely((tsock->flags & TSOCK_FLAG_FIN_SENT) != 0 && TCP_SEG(pkt)->ack == tsock->snd_nxt)) {
        ...
        if (acked_len > 0) {
            ...
            if (rtt) rtt_update(worker, tsock, rtt);
        ...
        if (opts->nr_sack) tcp_rcv_sack(worker, tsock, opts);

        handle_fast_retransmit(worker, tsock, pkt, acked_len);

        if (likely(tsock->retrans_stage == NONE)) {
            if (acked_len > 0) tsock->snd_recover = tsock->snd_una - 1;
        } else {
            /*
            ...
            if (seq_ge(TCP_SEG(pkt)->ack, tsock->snd_recover)) {
                ...
                tsock->retrans_stage = NONE;
                tsock_trace_archive(tsock->trace, "rto-%.3fms", (double)recover_time / 1e3);
            } else if (tsock->retrans_stage == RTO) {
                tcp_retrans(worker, tsock);
            }
        }

        if (tsock->snd_una == tsock->snd_nxt) timer_stop(&tsock->timer_rto);

        if (seq_lt(tsock->snd_wl1, TCP_SEG(pkt)->seq) ||
            (TCP_SEG(pkt)->seq == tsock->snd_wl1 && seq_le(tsock->snd_wl2, TCP_SEG(pkt)->ack))) {
...
static inline void tcp_rcv_ack_fastpath(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    int acked_len;

    debug_assert(tsock->state == TCP_STATE_ESTABLISHED);
    debug_assert(seq_ge(TCP_SEG(pkt)->ack, tsock->snd_una) && seq_le(TCP_SEG(pkt)->ack, tsock->snd_nxt));
...
static inline void update_ts_recent(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
...
static inline int tcp_rcv_fastpath(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    int err;

    /* disable fastpath when seq, ack and flags are not expected */
    if (unlikely(TCP_SEG(pkt)->seq != tsock->rcv_nxt || seq_lt(TCP_SEG(pkt)->ack, tsock->snd_una) ||
                 seq_gt(TCP_SEG(pkt)->ack, tsock->snd_nxt) || (TCP_SEG(pkt)->flags & ~TCP_FLAG_PSH) != TCP_FLAG_ACK ||
                 (TCP_SEG(pkt)->len == 0 && TCP_SEG(pkt)->ack == tsock->snd_una) || tsock->retrans_stage != NONE || tsock->rcv_wnd == 0))
        return 0;

    /* we also assume ts opt is a must */
    if (unlikely(TCP_SEG(pkt)->opt_len != TCP_OPT_TS_SPACE || !(pkt->flags & PKT_FLAG_HAS_TS_OPT))) return 0;

    /* light PAWS check; ts wrap will be checked in slowpath */
    if (seq_lt(TCP_SEG(pkt)->ts_val, tsock->ts_recent)) return 0;
    if (seq_le(TCP_SEG(pkt)->seq, tsock->last_ack_sent)) update_ts_recent(worker, tsock, pkt);

    err = tcp_rcv_data(worker, tsock, pkt);
    if (unlikely(err)) return err;

    trace_tcp_ts_opt(tsock, TCP_SEG(pkt)->ts_val, TCP_SEG(pkt)->ts_ecr, tsock->ts_recent, tsock->last_ack_sent);
...
static int validate_seq(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt, struct tcp_opts *opts) {
    uint32_t seq = TCP_SEG(pkt)->seq;
    uint32_t end = TCP_SEG(pkt)->seq + TCP_SEG(pkt)->len;

    if (TCP_SEG(pkt)->len == 0 && seq == tsock->rcv_nxt) return 0;

    if (seq_le(end, tsock->rcv_nxt) || seq_ge(seq, tsock->rcv_nxt + tsock->rcv_wnd)) return -ERR_TCP_INVALID_SEQ;

    return 0;
}

static int process_incoming_seq(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt, struct tcp_opts *opts) {
    int err;
    ...
    if (opts->has_ts) {
         ...
         *   to the PAWS check
         */
        if (seq_lt(opts->ts.val, tsock->ts_recent) && (now_in_sec(worker) - tsock->ts_recent_in_sec) < TCP_PAWS_IDLE_MAX &&
            !has_flag_rst(pkt))
            return -ERR_TCP_INVALID_TS;

        if (seq_le(TCP_SEG(pkt)->seq, tsock->last_ack_sent) && !has_flag_rst(pkt)) update_ts_recent(worker, tsock, pkt);
    }

    return 0;
}

static inline void process_tcp_opt_negotiation(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    ...
    trace_tcp_ts_opt(tsock, TCP_SEG(pkt)->ts_val, TCP_SEG(pkt)->ts_ecr, tsock->ts_recent, tsock->last_ack_sent);
    if (ret < 0) WORKER_TSOCK_STATS_INC(worker, tsock, -ret);
...
static int syn_sent_to_established(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
...
static __rte_noinline int tcp_rcv_rst(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    int ret = -WARN_RST_RECV;
    int to_close = 0;

    debug_assert(has_flag_rst(pkt));

    if (tsock->state == TCP_STATE_CLOSED) return ret;
...
static inline void passive_tsock_init(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt, void *data) {
    struct sock_key key;
    tsock->net_hdr_len = init_net_hdr_from_pkt(&tsock->net_hdr, &tsock->local_ip, &tsock->remote_ip, pkt);
    ...
    tsock->snd_isn = isn_gen(&tsock->local_ip, &tsock->remote_ip, tsock->local_port, tsock->remote_port);

    process_tcp_opt_negotiation(worker, tsock, pkt);
    tsock_set_state(tsock, TCP_STATE_SYN_RCVD);
}

static inline int tsock_accept(struct tpa_worker *worker, struct tcp_sock *listen_tsock, struct packet *pkt) {
    struct tcp_sock *tsock;

    debug_assert(tsock_lookup_slowpath(worker, pkt, &tsock) != 0);

    tsock = sock_create(NULL, !!(pkt->flags & PKT_FLAG_IS_IPV6));
    if (!tsock) return -ERR_TOO_MANY_SOCKS;
 ...
 * time.
 */
static inline int tcp_rcv_pkt_at_listen(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    int err;

    if (has_flag_rst(pkt)) return -WARN_RST_RECV;

    if (has_flag_ack(pkt)) {
        err = xmit_rst_for_listen(worker, tsock, pkt);
        if (err) WORKER_TSOCK_STATS_INC(worker, tsock, -err);

        return -WARN_ACK_AT_LISTEN;
    }

    if (has_flag_syn(pkt)) return tsock_accept(worker, tsock, pkt);

    return -WARN_INVLIAD_PKT_AT_LISTEN;
}

static int tcp_rcv_pkt_at_syn_sent(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    if (has_flag_ack(pkt)) {
     ...
     * segment and return
     */
    if (!has_flag_syn(pkt)) return -ERR_NO_SYN_AND_RST;

    /* XXX: handle simultaneous connect */
    if (!has_flag_ack(pkt)) return -SIMULTANEOUS_CONNECT;

    if (seq_gt(TCP_SEG(pkt)->ack, tsock->snd_isn)) return syn_sent_to_established(worker, tsock, pkt);

    return -ERR_INVALID_SYN_SENT_PROCESS;
}

static inline int tcp_rcv_fin(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    if (likely(!has_flag_fin(pkt))) return 0;

    tsock->flags |= TSOCK_FLAG_EOF;

    if (tsock->state == TCP_STATE_CLOSED || tsock->state == TCP_STATE_LISTEN || tsock->state == TCP_STATE_SYN_SENT)
 ...
 */
static inline int tcp_rcv_pkt(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    struct tcp_opts opts;
    int err = 0;
    ...
    if (unlikely(pkt->wid != worker->id && tsock->state != TCP_STATE_LISTEN)) return -WARN_STALE_PKT_WORKER_MISMATCH;

    if (likely(tsock->state == TCP_STATE_ESTABLISHED && tcp_rcv_fastpath(worker, tsock, pkt) == 1)) {
        tsock_rearm_timer_keepalive(tsock, worker->ts_us);
        return 0;
    }
    ...
    if (err < 0) WORKER_TSOCK_STATS_INC(worker, tsock, -err);

    /* first, check sequence number */
    err = process_incoming_seq(worker, tsock, pkt, &opts);
    if (err) goto send_ack;

    /* second, check the RST bit */
    if (has_flag_rst(pkt)) return tcp_rcv_rst(worker, tsock, pkt);
    ...
    if (tsock->state == TCP_STATE_ESTABLISHED || tsock->state == TCP_STATE_FIN_WAIT_1 || tsock->state == TCP_STATE_FIN_WAIT_2) {
        err = tcp_rcv_data(worker, tsock, pkt);
        if (err) return err;
    } else {
        if (TCP_SEG(pkt)->len != 0) return -ERR_TCP_RCV_INVALID_STATE;
    }
...
send_ack:
    if (!has_flag_rst(pkt)) {
        if (TCP_SEG(pkt)->len != 0 && seq_lt(TCP_SEG(pkt)->seq, tsock->rcv_nxt)) tsock_reset_quickack(tsock);
        tsock_set_ack_flag(tsock, TSOCK_FLAG_ACK_NOW);
    }
...
static inline void queue_input_tsock(struct tpa_worker *worker, struct tcp_sock *tsock, uint32_t *nr_tsock) {
    uint32_t i = *nr_tsock;

    if (i > 0 && tsock == worker->tsocks[i - 1]) return;

    worker->tsocks[i] = tsock;
    *nr_tsock += 1;
}

static inline void tcp_rcv_process(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    int err;

    vstats_add(&tsock->rx_merge_size, pkt->nr_read_seg);
    /* here we do count only when merge happened; therefore > 1 here */
    if (pkt->nr_read_seg > 1) WORKER_TSOCK_STATS_ADD(worker, tsock, PKT_RECV_MERGE, pkt->nr_read_seg);
    ...
    if (unlikely(err || TCP_SEG(pkt)->len == 0)) free_err_pkt(worker, tsock, pkt, err);

    if (unlikely(tsock->flags & TSOCK_FLAG_ACK_NOW)) xmit_flag_packet(worker, tsock);

    tsock->rx_merge_head = NULL;
}

static inline int has_ts_opt_only(struct packet *pkt) {
    return TCP_SEG(pkt)->opt_len == TCP_OPT_TS_SPACE && (pkt->flags & PKT_FLAG_HAS_TS_OPT);
}

static inline int tcp_can_merge(struct packet *head, struct packet *pkt) {
    /* XXX: need simplify this */
    return tcp_cfg.enable_rx_merge && has_ts_opt_only(pkt) && TCP_SEG(head)->len > 0 && TCP_SEG(pkt)->len > 0 &&
           (TCP_SEG(pkt)->flags & ~TCP_FLAG_PSH) == TCP_FLAG_ACK && TCP_SEG(head)->ack == TCP_SEG(pkt)->ack &&
           (TCP_SEG(head)->seq + TCP_SEG(head)->len == TCP_SEG(pkt)->seq) && TCP_SEG(head)->ts_raw == TCP_SEG(pkt)->ts_raw &&
           TCP_SEG(head)->len + pkt->l5_len < TSOCK_MAX_MERGE_SIZE;
}

static inline void tcp_merge(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    struct packet *head;
...
int tcp_input(struct tpa_worker *worker, struct packet **pkts, int nr_pkt) {
    ...
    for (i = 0; i < nr_tsock; i++) {
        ...
        if (tsock->rx_merge_head) tcp_rcv_process(worker, tsock, tsock->rx_merge_head);

        if (tsock->flags & TSOCK_FLAG_ACK_NEEDED) {
            if (tsock->flags & TSOCK_FLAG_ACK_NOW) {
                xmit_flag_packet(worker, tsock);
            } else {
                flex_fifo_push_if_not_exist(worker->delayed_ack, &tsock->delayed_ack_node);
            }
        }
