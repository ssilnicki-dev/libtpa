...
static struct neigh_entry *neigh_find_locked(struct tpa_ip *ip) {
    ...
    for (i = 0; i < neigh_cache.nr_neigh; i++) {
        ...
        if (tpa_ip_equal(&entry->ip, ip)) return entry;
    }
...
static void neigh_add(struct tpa_ip *ip, uint8_t *mac, struct neigh_entry *evicted) {
    ...
    if (idx == MAX_NEIGH_ENTRY) idx = neigh_evict(evicted);
    ...
    if (neigh_cache.nr_neigh < MAX_NEIGH_ENTRY) neigh_cache.nr_neigh += 1;
}

enum { NEIGH_NONE, NEIGH_ADD, NEIGH_UPDATE };
...
void neigh_update(struct tpa_ip *ip, uint8_t *mac) {
    ...
    if (tpa_ip_is_ipv4(ip)) {
        if (tpa_ip_get_ipv4(ip) == dev.gw4) neigh_cache.gw_neigh = entry;
    } else {
        if (tpa_ip_equal(ip, &dev.gw6.ip)) neigh_cache.gw6_neigh = entry;
    }

    if (op == NEIGH_NONE) return;

    /* do log outside spin lock */
    LOG("NEIGH %s: %s\t" MAC_FMT, op == NEIGH_ADD ? "add" : "update", ip_str, MAC_ARGS(mac));

    if (evicted.ip.u64[0] | evicted.ip.u64[1]) {
        LOG("NEIGH evict: %s\t" MAC_FMT, tpa_ip_to_str(&evicted.ip, ip_str, sizeof(ip_str)), MAC_ARGS(mac));
    }
}
...
static int do_neigh_wait_enqueue(struct packet *pkt) {
    return rte_ring_enqueue(neigh_cache.neigh_waiting_queue, pkt) == 0 ? 0 : -ERR_NEIGH_ENQUEUE;
}
...
static void try_drop_stale_neigh_request(void) {
    ...
    nr_pkt = rte_ring_dequeue_burst(neigh_cache.neigh_waiting_queue, (void **)pkts, NEIGH_QUEUE_LEN, NULL);

    /* FIXME: it's not efficient */
    for (i = 0; i < nr_pkt; i++) {
        if (!stale_tsock(pkts[i]->tsock)) do_neigh_wait_enqueue(pkts[i]);
    }
}

int neigh_wait_enqueue(struct packet *pkt) {
    if (do_neigh_wait_enqueue(pkt) == 0) return 0;
...
int neigh_flush(struct tpa_worker *worker) {
    ...
    for (i = 0; i < BATCH_SIZE; i++) {
        ...
        if (pkt->tsock) flush_tcp_packet(pkt, ret);
...
void neigh_handle_reply(struct tpa_ip *ip, uint8_t *mac) {
    ...
    entry = neigh_find(ip);
    if (entry || neigh_cache.accept_garp) neigh_update(ip, mac);

    if (entry) check_neigh_wait_queue();
}
...
static void neigh_impl_init(void) {
    ...
    for (i = 0; i < NR_NEIGH_IMPL; i++) {
        ...
        fd = impl->ops->nd_init();
        if (fd == ND_SKIP) continue;

        if (fd < 0) {
            LOG_ERR("failed to create poll socket for %s: %s", impl->name, strerror(errno));
            continue;
        }
...
void neigh_dump(struct shell_buf *reply) {
    ...
    for (i = 0; i < neigh_cache.nr_neigh; i++) {
        ...
        shell_append_reply(reply, "%s\t" MAC_FMT "\n", tpa_ip_to_str(&entry->ip, buf, sizeof(buf)), MAC_ARGS(entry->mac.addr_bytes));
    }
}
...
static int cmd_neigh(struct shell_cmd_info *cmd) {
    ...
    if (cmd->argc == 1 && strcmp(cmd->argv[0], "-s") == 0) {
        shell_append_reply(cmd->reply,
                           ...
                           "NDP.rx_pkts: %lu\n"
                           "NDP.tx_pkts: %lu\n",
                           neigh_impl[0].rx_pkts, neigh_impl[0].tx_pkts, neigh_impl[1].rx_pkts, neigh_impl[1].tx_pkts);
    }
...
void neigh_init(void) {
    ...
    if (getenv("TPA_ARP_ACCEPT_GARP")) neigh_cache.accept_garp = 1;
}
