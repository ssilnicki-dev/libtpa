...
static const char *map_type_to_str[] = {
    [MAP_TYPE_DST_IP4] = "dst_ip4_map", [MAP_TYPE_DST_IP6] = "dst_ip6_map", [MAP_TYPE_DST_PORT] = "dst_port_map",
    [MAP_TYPE_XSKS] = "xsks_map",       [MAP_TYPE_STATS] = "xdp_stats",
};
...
static int xdp_map_type_find(const char *name) {
    ...
    for (i = 0; i < MAP_TYPE_MAX; i++) {
        if (MATCH(map_type_to_str[i], name)) return i;
    }
...
static int xdp_prog_init(void) {
    if (xdp_prog_query(dev.name, &xdp_prog) < 0) return -1;

    if (xdp_prog_maps_init(&xdp_prog, xdp_prog.info.nr_map_ids) < 0) return -1;
...
static int do_xdp_map_update(int type, void *key, void *value) {
    ...
    if (unlikely(err)) {
        LOG_ERR("failed to update map elem for %s: %s", map_type_to_str[type], strerror(errno));
        return -1;
    }
...
static int do_xdp_map_delete(int type, void *key) {
    ...
    if (unlikely(err)) {
        LOG_ERR("failed to delete map elem for %s: %s", map_type_to_str[type], strerror(errno));
        return -1;
    }
...
int xdp_ctrl_init(void) {
    if (xdp_prog_init() < 0) return -1;

    if (xdp_flow_add_ip_rules() < 0) return -1;
...
int xdp_flow_add_port_rules(uint16_t port) {
    ...
    if (do_xdp_map_update(MAP_TYPE_DST_PORT, &port, &queue) < 0) return -1;
...
int xdp_flow_remove_port_rules(uint16_t port) {
    ...
    if (do_xdp_map_delete(MAP_TYPE_DST_PORT, &port) < 0) return -1;
...
struct rte_flow *xdp_flow_create(uint16_t port_id, const struct rte_flow_attr *attr, const struct rte_flow_item pattern[],
    ...
    do {
        item = &pattern[i++];
        if (item->type == RTE_FLOW_ITEM_TYPE_TCP) break;
    } while (item->type != RTE_FLOW_ITEM_TYPE_END);

    if (item->type != RTE_FLOW_ITEM_TYPE_TCP) goto fail;
...
int xdp_prog_detach(const char *dev_name) {
    ...
    if (xdp_prog_query(dev_name, &prog) < 0) return -1;

    if (strncmp(prog.name, XDP_PROG_NAME_PREFIX, strlen(XDP_PROG_NAME_PREFIX)) != 0) return -1;
