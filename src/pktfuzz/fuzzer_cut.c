...
static inline int get_cut_size(struct fuzz_cut_cfg *cut, uint16_t payload_len) {
    if (cut->size.random) return (rte_rdtsc() % payload_len) / (cut->head + cut->tail);
...
#define relative_snd_seq(seq, pkt) seq - pkt->tsock->snd_isn
...
static void cut_tail(struct packet *pkt, int size) {
    ...
    pkt->mbuf.pkt_len -= size;
    seq = ntohl(tcp->sent_seq) + pkt->mbuf.pkt_len - pkt->hdr_len, trace_pktfuzz_cut(pkt->tsock, seq, size);
...
static inline void do_cut(struct port_txq *txq, struct fuzz_cut_cfg *cut, int payload_len) {
    ...
    if (size) {
        if (cut->head) pkt = cut_head(pkt, size);
        if (cut->tail) cut_tail(pkt, size);
    }
...
static void cut_fuzz(struct port_txq *txq) {
    ...
    if (!cut->enabled || txq->nr_pkt == 0) return;

    pkt = txq->pkts[txq->nr_pkt - 1];
    payload_len = pkt->mbuf.pkt_len - pkt->hdr_len;
    if (payload_len == 0) return;

    if (pkt->tsock == NULL || pkt->tsock->snd_mss == 0) return;

    mss = pkt->tsock->snd_mss;
    nr_seg = (payload_len + mss - 1) / mss;
    if (meet_rate_n(&cut->rate, nr_seg)) do_cut(txq, cut, payload_len);
}
...
static void cut_stats(struct shell_buf *reply, struct fuzz_cfg *fuzz_cfg) {
    ...
    shell_append_reply(reply,
                       "cut:\n"
                       "\tenabled: %d\n" RATE_FMT "\tstats.total: %lu\n",
                       cut->enabled, RATE_ARGS(&cut->rate), cut->stats.total);
}

static void cut_help(struct shell_buf *reply, struct fuzz_cfg *fuzz_cfg) {
    shell_append_reply(reply, "cut           cut few bytes of the packet payload\n"
...
static int cut_parse(struct fuzz_opt *opts) {
    ...
    if (cut->head == 0 && cut->tail == 0) cut->tail = 1;

    if (!num_given(&cut->size)) cut->size.num = 1;
