...
static inline int do_port_alloc(uint16_t port) {
    ...
    if (port_allocated(port)) return 0;
...
uint16_t port_alloc(uint16_t port) {
    ...
    if (port) return do_port_alloc(port);

    port = (rte_rdtsc() % PORT_COUNT) + pac.min;
    while (nr_try++ < PORT_COUNT) {
        if (do_port_alloc(port) > 0) return port;

        port += 1;
        if (port == pac.max) port = pac.min;
    }
...
static inline void port_block_put(struct port_block *block) {
    ...
    if (--block->refcnt == 0) timer_start(&block->timer, block->worker->ts_us, tcp_cfg.time_wait);
}
...
static struct port_block *port_block_find(struct port_block **blocks, int nr_block, uint16_t port) {
    ...
    for (i = 0; i < nr_block; i++) {
        if (port >= blocks[i]->start && port < blocks[i]->end) return blocks[i];
    }
...
static inline int port_range_is_free(uint16_t start, uint16_t size) {
    ...
    for (i = 0; i < size; i++)
        if (pac.port_map[start + i] >= 0) return 0;
...
static struct port_block *do_port_block_alloc(struct tpa_worker *worker, uint16_t start, uint16_t size) {
    ...
    if (!port_range_is_free(start, size)) return NULL;
    ...
    timer_init(&block->timer, &worker->timer_ctrl, port_block_timeout, block, worker->ts_us);
...
out:
    ...
    if (block) LOG("allocated port block %hu-%hu", block->start, block->end);
...
static void reserve_local_port(void) {
    ...
    if (range[0] > PORT_MIN) {
        LOG_WARN("got abnormal kernel local port range: %d %d; skip", range[0], range[1]);
        goto out;
    }

    if (range[1] != PORT_MIN - 1) {
        LOG("update kernel port range: %d %d -> %d %d", range[0], range[1], range[0], PORT_MIN - 1);

        rewind(f);
        if (fprintf(f, "%d %d", range[0], PORT_MIN - 1) <= 0) {
            LOG_WARN("failed to update kernel local port range: %s", strerror(errno));
        }
    }
out:
...
int local_port_range_set(struct cfg_spec *spec, const char *val) {
    ...
    if (min < PORT_MIN || min >= PORT_MAX || max < PORT_MIN || max >= PORT_MAX) {
        LOG_WARN("local_port_range [%d %d] is out of range [%d %d]", min, max, PORT_MIN, PORT_MAX);
        return -1;
    }
...
static int cmd_port_alloc(struct shell_cmd_info *cmd) {
    ...
    if (cmd->argc == 1 && strcmp(cmd->argv[0], "-v") == 0) list_port(cmd->reply);
 ...
 */
static uint16_t bind_from_port_block(struct port_block *block, struct tpa_worker *worker, struct sock_key *key, struct tcp_sock *tsock) {
    int nr_try = 0;
    int idx;
...
static struct port_block *worker_alloc_port_block(struct tpa_worker *worker, uint16_t port) {
    ...
    if (worker->nr_port_block >= MAX_PORT_BLOCK_PER_WORKER) return NULL;
...
static uint16_t port_bind_on_given_port(struct tpa_worker *worker, struct sock_key *key, struct tcp_sock *tsock) {
    ...
    if (sock_table_add(&worker->sock_table, key, tsock) < 0) return 0;
...
static uint16_t port_bind_on_random_port(struct tpa_worker *worker, struct sock_key *key, struct tcp_sock *tsock) {
    struct port_block *block;
    uint16_t port;
    int nr_try = 0;
    int idx;

    if (worker->nr_port_block == 0) goto alloc;
    ...
    while (nr_try++ < worker->nr_port_block) {
        ...
        idx += 1;
        if (idx >= worker->nr_port_block) idx = 0;
    }
...
int port_unbind(struct tpa_worker *worker, struct sock_key *key) {
    ...
    if (sock_table_del(&worker->sock_table, key) < 0) return -1;
