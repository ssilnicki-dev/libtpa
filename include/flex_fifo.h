...
static inline int _ff_ring_push(struct flex_fifo *ff, struct flex_fifo_node *node) {
    if (unlikely(!TAILQ_EMPTY(&ff->list) || flex_fifo_ring_free_size(ff) == 0)) return -1;
...
static inline struct flex_fifo_node *_ff_ring_pop(struct flex_fifo *ff) {
    ...
    if (unlikely(ff->read == ff->write)) return NULL;
...
static inline struct flex_fifo_node *_ff_list_pop(struct flex_fifo *ff) {
    ...
    if (node) _ff_do_list_remove(ff, node);
...
static inline void *flex_fifo_pop_entry(struct flex_fifo *ff, size_t offset) {
    ...
    if (likely(node)) return (void *)((char *)node - offset);

    return NULL;
}

#define FLEX_FIFO_POP_ENTRY(ff, type, member) (type *)flex_fifo_pop_entry(ff, offsetof(type, member))
...
#define FLEX_FIFO_PEEK_ENTRY(ff, type, member) (type *)flex_fifo_peek_entry(ff, offsetof(type, member))

static inline int flex_fifo_remove(struct flex_fifo *ff, struct flex_fifo_node *node) {
    if (_ff_ring_remove(ff, node) == 0) return 0;
...
static inline struct flex_fifo *flex_fifo_create(uint32_t size) {
    ...
    if ((size & (size - 1)) || size > RING_MAX_SIZE) return NULL;
...
#define FLEX_FIFO_POP_ENTRY_LOCK(ff, type, member) (type *)flex_fifo_pop_entry_lock(ff, offsetof(type, member))
...
#define FLEX_FIFO_PEEK_ENTRY_LOCK(ff, type, member) (type *)flex_fifo_peek_entry_lock(ff, offsetof(type, member))
