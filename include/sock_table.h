...
static inline void sock_key_init(struct sock_key *key, struct tpa_ip *remote_ip, uint16_t remote_port, struct tpa_ip *local_ip,
                                 uint16_t local_port) {
...
static inline int sock_key_equal(struct sock_key *a, struct sock_key *b) {
    return tpa_ip_equal(&a->local_ip, &b->local_ip) && tpa_ip_equal(&a->remote_ip, &b->remote_ip) && a->port == b->port;
}
...
static inline uint32_t sock_hash_idx(struct sock_key *key) {
    ...
    hash = rte_crc32_u64(key->remote_ip.u64[0] | (uint64_t)key->local_port, key->remote_ip.u64[1] | (uint64_t)key->remote_port);
...
static inline struct tcp_sock *sock_table_lookup(struct sock_table *table, struct sock_key *key) {
    ...
    TAILQ_FOREACH(entry, list, node) {
        if (sock_key_equal(&entry->key, key)) return entry->tsock;
    }
...
static inline int sock_table_add(struct sock_table *table, struct sock_key *key, struct tcp_sock *tsock) {
    ...
    if (sock_table_lookup(table, key)) return -1;
