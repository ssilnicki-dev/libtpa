...
#define RATE_ARGS(rate) rate_to_str(rate), (rate)->total ? (double)((rate)->hit) * 100 / (rate)->total : 0, (rate)->hit, (rate)->total
...
static inline void pktfuzz(struct port_txq *txq) {
    if (unlikely(pktfuzz_enabled)) fuzz(txq);
}

static inline void pktfuzz_run(struct port_txq *txq) {
    if (unlikely(pktfuzz_enabled)) fuzz_run(txq);
}
