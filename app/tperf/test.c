...
static int read_test_info(struct connection *conn, struct tpa_iovec *iov, int size) {
    ...
    /* if already parsed? */
    if (off == sizeof(struct test_info)) return 0;
    ...
    conn->info_off = off;
    if (off == sizeof(struct test_info)) init_server_conn(conn);

    return bytes_eaten;
}

static void read_test_data(struct connection *conn, struct tpa_iovec *iov, int bytes_read, int bytes_eaten) {
    char *base;
    int len;
    int sum = 0;
    int i = 0;
    ...
    while (sum < bytes_read) {
        ...
        if (sum < bytes_eaten) {
            ...
            if (sum + len < bytes_eaten) goto next;

            base = iov[i].iov_base + bytes_eaten - sum;
            len -= bytes_eaten - sum;
        }

        if (conn->integrity_enabled) integrity_verify(base, len, conn->integrity_off + conn->stats.bytes_read);
...
static void on_rr_read_done(struct connection *conn) {
    ...
    if (conn->is_client) {
        ...
        if (conn->test == TEST_CRR) conn->to_close = 1;
    }
...
static void on_read_done(struct connection *conn) {
    if (conn->read.off < conn->read.budget) return;
...
static int emit_test_info(struct connection *conn) {
    ...
    if (conn->info_off == sizeof(struct test_info)) return 0;
...
static int setup_test_data(struct test_thread *thread, struct connection *conn, struct tpa_iovec *iov) {
    ...
    while (off < budget) {
        ...
        if (conn->integrity_enabled) integrity_fill(mbuf->data, len, conn->integrity_off + conn->stats.bytes_write + off);
...
static void on_write_done(struct connection *conn, int bytes_write) {
    ...
    if (conn->write.off < conn->write.budget) return;
    assert(conn->write.off == conn->write.budget);

    /* disable futher writes unless we get the response */
    if (conn->test == TEST_RR || conn->test == TEST_CRR) conn->write.budget = 0;
...
int conn_on_write(struct connection *conn) {
    ...
    if (ctx.is_client && emit_test_info(conn) < 0) return -1;
