...
static inline int get_dup_count(struct fuzz_num *nr_pkt, int max) {
    if (nr_pkt->random) return (rte_rdtsc() / 3) % max;
...
static inline void do_dup(struct port_txq *txq, struct fuzz_dup_cfg *dup) {
    ...
    if (dup->syn && (tcp->tcp_flags & RTE_TCP_SYN_FLAG) == 0) return;
...
static void dup_fuzz(struct port_txq *txq) {
    ...
    if (meet_rate(&dup->rate)) do_dup(txq, dup);
}
...
static void dup_stats(struct shell_buf *reply, struct fuzz_cfg *fuzz_cfg) {
    ...
    shell_append_reply(reply,
                       ...
                       "\tsyn: %d\n"
                       "\tstats.total: %lu\n",
                       dup->enabled, RATE_ARGS(&dup->rate), num_to_str(&dup->nr_pkt), dup->syn, dup->stats.total);
}

static void dup_help(struct shell_buf *reply, struct fuzz_cfg *fuzz_cfg) {
    shell_append_reply(reply, "dup           duplicate a packet\n"
...
static int dup_parse(struct fuzz_opt *opts) {
    ...
    if (!num_given(&dup->nr_pkt)) dup->nr_pkt.num = 1;
