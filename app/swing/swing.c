...
static int loop(void) {
    if (watch_stdin() < 0) return -1;

    if (watch_connection() < 0) return -1;

    while (1) {
        tpa_worker_run(worker);

        if (poll_stdin() < 0) return -1;

        if (poll_connection() < 0) return -1;
...
static void usage(void) {
    fprintf(stderr, "usage: swing [options] server port\n"
                    "\n"
