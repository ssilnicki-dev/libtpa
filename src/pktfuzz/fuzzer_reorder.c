...
static void reorder_fuzz(struct port_txq *txq) {
    ...
    if (meet_rate(&reorder->rate)) do_reorder(txq, reorder);
}
...
static void reorder_stats(struct shell_buf *reply, struct fuzz_cfg *fuzz_cfg) {
    ...
    shell_append_reply(reply,
                       "reorder:\n"
                       "\tenabled: %d\n" RATE_FMT "\treordered: %lu\n",
                       reorder->enabled, RATE_ARGS(&reorder->rate), reorder->stats.reordered);
}

static void reorder_help(struct shell_buf *reply, struct fuzz_cfg *fuzz_cfg) {
    shell_append_reply(reply, "reorder       reorder packets\n"
