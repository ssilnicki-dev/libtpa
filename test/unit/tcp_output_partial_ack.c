...
static void test_tcp_output_partial_ack_overlap(void) {
    ...
    ack += 700;
    pkts[1] = ut_inject_ack_packet(tsock, ack);
    ack += 700;
    pkts[2] = ut_inject_ack_packet(tsock, ack);
    ack = tsock->snd_nxt;
...
#define NR_PKT 10
    ...
    printf("un-acked=%u txq_active_count=%u\n", (uint32_t)(tsock->snd_nxt - tsock->snd_una), tcp_txq_unfinished_pkts(&tsock->txq));
    ut_close(tsock, CLOSE_TYPE_4WAY);
}
