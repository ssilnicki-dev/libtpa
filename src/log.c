...
static int log_file_prefix(char *buf, int size) {
    ...
    len += tpa_snprintf(buf + len, size - len, ".%06lu [%d %s] ", now.tv_usec % 1000000, log_ctrl.pid, log_ctrl.program);
...
static void log_file_write(char *buf, int len, int size) {
    ...
    if (ret != len) {
        syslog(LOG_WARNING, "failed to write to log file: %s: %s; fallback to syslog", log_ctrl.file, strerror(errno));
        log_ctrl.fd = -1;
    }
}
...
static void log_file_init(void) {
    ...
    if (fd < 0) {
        tpa_log(LOG_LEVEL_WARN, "failed to open log file: %s: %s", file, strerror(errno));
        return;
    }
...
void tpa_log(int level, const char *fmt, ...) {
    ...
    if (level > log_ctrl.level || getenv("TPA_LOG_DISABLE")) return;

    va_start(ap, fmt);
    if (log_ctrl.fd >= 0) len = log_file_prefix(buf, sizeof(buf));
