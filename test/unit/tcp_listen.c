...
static struct tcp_sock *accept_one_tsock(int expected_sid, void *data) {
    ...
    assert(tpa_accept_burst(worker, &sid, 1) == 1);
    {
        ...
        if (expected_sid >= 0) assert(sid == expected_sid);
...
static int listen_random_port(const char *local, void *data) {
    ...
    while (1) {
        ...
        if (errno != EADDRINUSE) return sid;
    }

    return -1;
}

static struct tcp_sock *do_tcp_listen_one(int port, struct tcp_sock **listen_tsock_ptr, int drain_accept_queue) {
    ...
    if (drain_accept_queue) assert(accept_one_tsock(tsock->sid, data) == tsock);
