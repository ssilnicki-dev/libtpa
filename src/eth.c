...
static __rte_noinline void parse_packet_type(struct packet *pkt) {
    ...
    case RTE_ETHER_TYPE_IPV4:
        ...
        if (ip->next_proto_id == IPPROTO_TCP) m->packet_type |= RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP;
        break;

    case RTE_ETHER_TYPE_IPV6:
        ip6 = packet_ip6_hdr(pkt);
        if (ip6->proto == IPPROTO_TCP) m->packet_type |= RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP;
        break;
...
static __rte_noinline int verify_csum(struct packet *pkt) {
    ...
    if (!(pkt->flags & PKT_FLAG_IS_IPV6)) {
        ...
        if (csum != 0) return -ERR_BAD_CSUM_IPV4;
    }

    csum = calc_udptcp_csum(pkt, packet_ip_hdr(pkt));
    if (csum != 0) return -ERR_BAD_CSUM_TCP;
...
int parse_eth_ip(struct packet *pkt) {
    ...
    if (unlikely(!(dev.caps & RX_OFFLOAD_PACKET_TYPE))) parse_packet_type(pkt);

    if ((m->packet_type & (RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP)) == (RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_TCP)) {
        struct rte_ipv4_hdr *ip = packet_ip_hdr(pkt);

        /* TODO: handle ip frags */
        if (unlikely(ip_is_frag(ip->fragment_offset))) return -PKT_IP_FRAG;
    ...
    } else if ((m->packet_type & (RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP)) == (RTE_PTYPE_L3_IPV6 | RTE_PTYPE_L4_TCP)) {
        struct rte_ipv6_hdr *ip = packet_ip6_hdr(pkt);

        if (unlikely(ip->proto != IPPROTO_TCP)) return -ERR_PKT_HAS_IPV6_OPT;
...
static __rte_noinline void misc_input(struct tpa_worker *worker, struct packet *pkt) {
    ...
    if (m->packet_type == RTE_PTYPE_L2_ETHER_ARP) err = arp_input(worker, pkt);
