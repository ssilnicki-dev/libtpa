 ...
 */
static inline void packet_attach_extbuf(struct packet *pkt, void *virt_addr, uint64_t phys_addr, uint16_t data_len) {
...
static inline struct packet *do_packet_alloc(struct rte_mempool *mempool) {
    ...
    if (unlikely(mempool == NULL)) return NULL;

    pkt = (struct packet *)rte_pktmbuf_alloc(mempool);
    if (likely(pkt != NULL)) packet_init(pkt);
...
static inline struct packet *packet_alloc(struct packet_pool *pool) {
    ...
    if (unlikely(pkt == NULL)) pkt = do_packet_alloc(backup_mempool(pool));
...
static inline void tcp_packet_cut(struct packet *pkt, uint32_t size, int dir) {
     ...
     */
    if (unlikely(pkt->flags & PKT_FLAG_VERIFY_CUT)) tcp_packet_cut_verify(pkt);
}
...
static inline struct packet *packet_linearize(struct packet *pkt, struct packet_pool *pool) {
    ...
    if (pkt->mbuf.nb_segs <= 1) return pkt;
