...
static inline void tsock_rearm_timer_keepalive(struct tcp_sock *tsock, uint64_t now) {
    ...
    if (tcp_cfg.keepalive) timer_start(&tsock->timer_keepalive, now, tcp_cfg.keepalive);
}
...
static inline char *get_flow_name(struct tcp_sock *tsock, char *name, size_t size) {
    ...
    tpa_snprintf(name, size, "%d %s:%hu -> %s:%hu", tsock->sid, local_ip, ntohs(tsock->local_port), remote_ip, ntohs(tsock->remote_port));

    return name;
}

static inline int init_net_hdr(struct eth_ip_hdr *hdr, struct rte_ether_hdr *eth, struct tpa_ip *local_ip, struct tpa_ip *remote_ip) {
    hdr->eth = *eth;
...
static inline void init_tpa_ip_from_pkt(struct packet *pkt, struct tpa_ip *src_ip, struct tpa_ip *dst_ip) {
...
static inline int init_net_hdr_from_pkt(struct eth_ip_hdr *net_hdr, struct tpa_ip *local_ip, struct tpa_ip *remote_ip, struct packet *pkt) {
...
static inline void tcp_reset_retrans(struct tcp_sock *tsock, uint32_t seq, uint16_t desc_base) {
...
static inline struct tcp_sock *tsock_get_by_sid(int sid) {
    ...
    if (unlikely((uint32_t)sid >= tcp_cfg.nr_max_sock)) return NULL;

    tsock = &sock_ctrl->socks[sid];
    if (unlikely(tsock->sid < 0)) return NULL;
...
static inline int parse_flow_mark(int wid, struct packet *pkt) {
    ...
    if (unlikely((pkt->mbuf.ol_flags & PKT_RX_FDIR_ID) == 0)) return -WARN_MISSING_FLOW_MARK;

    sid = pkt->mbuf.hash.fdir.hi >> tpa_cfg.nr_worker_shift;
    pkt->wid = pkt->mbuf.hash.fdir.hi & tpa_cfg.nr_worker_mask;

    if (unlikely(pkt->wid >= tpa_cfg.nr_worker)) return -ERR_FLOW_MARK_INVALID;
...
static inline int tuple_matches(struct tcp_sock *tsock, struct packet *pkt) {
    ...
    if (unlikely(tsock->state == TCP_STATE_LISTEN)) {
        if (unlikely(tsock->local_port != pkt->dst_port)) return 0;
    ...
    return tsock->local_port == pkt->dst_port && tsock->remote_port == pkt->src_port && tpa_ip_equal(&tsock->local_ip, &dst_ip) &&
           tpa_ip_equal(&tsock->remote_ip, &src_ip);
}

static inline int tsock_lookup(struct tpa_worker *worker, int wid, struct packet *pkt, struct tcp_sock **tsock_ptr) {
    struct tcp_sock *tsock;
    int sid;
    int err;
    ...
    if (likely(sid >= 0)) {
        if (unlikely(sid >= tcp_cfg.nr_max_sock)) return -ERR_NO_SOCK;

        tsock = &sock_ctrl->socks[sid];
        if (unlikely(!tuple_matches(tsock, pkt) || tsock->sid != sid)) return -WARN_STALE_PKT_TUPLE_MISMATCH;
    ...
    if (err == -WARN_MISSING_FLOW_MARK) {
        ...
        if (err == -ERR_NO_SOCK) err = listen_tsock_lookup(pkt, tsock_ptr);
    }
