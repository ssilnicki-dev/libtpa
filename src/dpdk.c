...
static struct nic_spec nic_unknow = {.name = "unknown", .type = NIC_TYPE_UNKNOWN, .rx_burst_cap = 32};
...
static void show_port_caps(struct shell_buf *reply, int port_id) {
    ...
    if (port->caps & TX_OFFLOAD_IPV4_CKSUM) shell_append_reply(reply, "TX_IPV4_CKSUM ");

    if (port->caps & TX_OFFLOAD_TCP_CKSUM) shell_append_reply(reply, "TX_TCP_CKSUM ");

    if (port->caps & TX_OFFLOAD_TSO) shell_append_reply(reply, "TSO ");

    if (port->caps & TX_OFFLOAD_MULTI_SEG) shell_append_reply(reply, "TX_MULTI_SEG ");

    if (port->caps & TX_OFFLOAD_PSEUDO_HDR_CKSUM) shell_append_reply(reply, "TX_PSEUDO_HDR_CKSUM ");

    if (port->caps & RX_OFFLOAD_PACKET_TYPE) shell_append_reply(reply, "RX_PACKET_TYPE ");

    if (port->caps & FLOW_OFFLOAD) shell_append_reply(reply, "FLOW_OFFLOAD ");

    if (port->caps & EXTERNAL_MEM_REGISTRATION) shell_append_reply(reply, "EXTMEM_REG ");
...
static void do_port_stats_get(struct shell_buf *reply, int port_id) {
    ...
    shell_append_reply(reply, "PORT[%d](%s):\n", port_id, dev.ports[port_id].state == PORT_LINK_UP ? "UP" : "DOWN");
...
static void port_stats_get(struct shell_buf *reply, struct shell_cmd_info *cmd) {
    ...
    if (dev.nic == NIC_TYPE_IAVF) pthread_mutex_lock(&dev.mutex);
...
out:
    if (dev.nic == NIC_TYPE_IAVF) pthread_mutex_unlock(&dev.mutex);
}

static void usage(struct shell_cmd_info *cmd) {
    shell_append_reply(cmd->reply, "usage: port cmd\n"
...
struct dpdk_port *get_dpdk_port_by_name(const char *name) {
    ...
    for (i = 0; i < tpa_cfg.nr_dpdk_port; i++) {
        if (strcmp(dev.ports[i].name, name) == 0) return &dev.ports[i];
    }
...
void set_dpdk_port_name(uint32_t port_id, const char *name) {
    ...
    if (port_id >= tpa_cfg.nr_dpdk_port) return;
...
static struct nic_spec *nic_spec_find(int port_id) {
    ...
    if (rte_eth_dev_info_get(port_id, &dev_info) < 0) goto out;

    for (i = 0; i < ARRAY_SIZE(nic_spec_list); i++) {
        if (strcmp(dev_info.driver_name, nic_spec_list[i].name) == 0) return &nic_spec_list[i];
    }
...
static uint32_t translate_caps(uint64_t dpdk_offloads, int nic_type) {
    ...
    if (dpdk_offloads & DEV_TX_OFFLOAD_IPV4_CKSUM) ret |= TX_OFFLOAD_IPV4_CKSUM;

    if (dpdk_offloads & DEV_TX_OFFLOAD_TCP_CKSUM) ret |= TX_OFFLOAD_TCP_CKSUM;

    if (dpdk_offloads & DEV_TX_OFFLOAD_TCP_TSO) ret |= TX_OFFLOAD_TSO;

    if (dpdk_offloads & DEV_TX_OFFLOAD_MULTI_SEGS) ret |= TX_OFFLOAD_MULTI_SEG;
...
static void dpdk_port_start(struct dpdk_port *port) {
    ...
    LOG("init port %hu: nr_queue=%hu rx_offload=%lu tx_offload=%lu", port_id, nr_queue, port_conf.rxmode.offloads,
        port_conf.txmode.offloads);

    ret = rte_eth_dev_configure(port_id, nr_queue, nr_queue, &port_conf);
    if (ret != 0) rte_panic("failed to configure device: %d", ret);

    ret = rte_eth_dev_adjust_nb_rx_tx_desc(port_id, &nr_rx_desc, &nr_tx_desc);
    if (ret < 0) rte_panic("failed to adjust number of rx tx desc: %d\n", ret);

    ret = rte_eth_macaddr_get(port_id, &mac);
    if (ret < 0) rte_panic("failed to get mac address for port %hu: %d\n", port_id, ret);

    rxq_conf = dev_info.default_rxconf;
    rxq_conf.offloads = port_conf.rxmode.offloads;
    mempool = packet_pool_get_mempool(generic_pkt_pool);
    for (i = 0; i < nr_queue; i++) {
        ret = rte_eth_rx_queue_setup(port_id, i, nr_rx_desc, socket_id, &rxq_conf, mempool);
        if (ret < 0) rte_panic("failed to setup rx queue %hu: %d", i, ret);
    }
    ...
    for (i = 0; i < nr_queue; i++) {
        ...
        if (ret < 0) rte_panic("failed to setup tx queue %u: %d", i, ret);
    }

    rte_flow_isolate(port_id, 1, NULL);

    if (rte_eth_dev_start(port_id) < 0) rte_panic("failed to start dpdk port %hu", port_id);

    nic_spec = nic_spec_find(port_id);

#if RTE_VERSION < RTE_VERSION_NUM(22, 11, 0, 0)
    if (nic_spec->type == NIC_TYPE_IAVF) rte_panic("IAVF requires DPDK v22.11 or above");
#endif
    ...
    LOG("started port %hu %s <drv_name=%s> %02X:%02X:%02X:%02X:%02X:%02X", port_id, port->device_id, nic_spec->name, mac.addr_bytes[0],
...
static struct rte_mempool *alloc_mempool(double percent, uint32_t mbuf_size, const char *name, uint32_t numa) {
    ...
    pool =
        rte_pktmbuf_pool_create(name, nr_mbuf, dpdk_cfg.mbuf_cache_size, sizeof(struct packet) - sizeof(struct rte_mbuf), mbuf_size, numa);
    if (!pool) LOG_ERR("failed to create mbuf pool: %s", name);

    return pool;
}

int packet_pool_create(struct packet_pool *pool, double percent, uint32_t mbuf_size, const char *fmt, ...) {
...
static void set_preferred_num(void) {
    ...
    if (node < 0 || node >= TPA_MAX_NUMA) {
        ...
        if (node < 0 || node >= TPA_MAX_NUMA) node = 0;
...
static int get_max_rx_pkt_len(void) {
    ...
    if (max_rx_pkt > hw_rx_capa) {
        LOG_WARN("warning: set max rx pkt len (%d) > hw capa (%d)", max_rx_pkt, hw_rx_capa);
...
static void mbuf_mempool_init(void) {
    ...
    mbuf_size = RTE_MAX(RTE_MBUF_DEFAULT_DATAROOM, max_rx_pkt_len + sizeof(struct rte_mbuf));
...
static int has_dpdk_arg_m(struct dpdk_args *args) {
    ...
    for (i = 0; i < args->argc; i++) {
        if (strcmp(args->argv[i], "-m") == 0) return 1;
    }
...
static void dpdk_args_set_mem(struct dpdk_args *args) {
    ...
    if (dpdk_cfg.huge_unlink) dpdk_args_push_one(args, "--huge-unlink");
}
...
static int parse_dpdk_args(struct dpdk_args *args) {
    ...
    if (tpa_master_core == NULL) tpa_master_core = "0";
...
static void eal_init(void) {
    ...
    if (rte_eal_init(args.argc, args.argv) < 0) rte_panic("failed to init DPDK");
     ...
     */
    if (restore_affinity && pthread_setaffinity_np(tid, sizeof(cpuset), &cpuset)) LOG_ERR("failed to restore affinity");
...
static void mempool_walk_func(struct rte_mempool *mp, void *arg) {
    ...
    for (i = 0; i < RTE_MAX_LCORE; i++) {
        if (mp->local_cache[i].len) shell_append_reply(reply, "%d/%-4d ", i, mp->local_cache[i].len);
    }
    shell_append_reply(reply, "\n");
}
...
static int show_mempool_stats(struct shell_buf *reply, int verbose) {
    ...
    if (verbose) rte_mempool_walk(walk_func_long, reply);
...
static void show_rte_malloc_stats(struct shell_buf *reply, int verbose) {
    ...
    while (fgets(line, sizeof(line), f)) {
        if (sscanf(line, "Heap id:%d", &id) == 1) {
            if (verbose == 0 && id >= numa_num_configured_nodes()) break;
        }
...
static int dump_one_memseg(const struct rte_memseg_list *msl, const struct rte_memseg *ms, size_t len, void *arg) {
    struct shell_buf *reply = arg;

    shell_append_reply(reply, "\tbase=%p size=%lu pagesize=%lu nr_page=%lu socket=%d external=%s\n", msl->base_va, len, msl->page_sz,
                       len / msl->page_sz, msl->socket_id, msl->external ? "yes" : "no");
    return 0;
}
...
void show_dpdk_mem_stats(struct shell_buf *reply, int verbose) {
    ...
    if (verbose) show_memzone_stats(reply);
}
