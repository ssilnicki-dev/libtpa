...
static struct archive_record *archive_record_alloc(void) {
    ...
    if (!record_free_list) return NULL;
...
static void archive_record_free(struct archive_record *record) {
    ...
    if (record_free_list) rte_ring_enqueue(record_free_list, record);
}

static void queue_to_archive_list(struct archive_record *record) {
    if (!record_to_archive_list) return;
...
static void flush_one_archive_record(struct archive_record *record) {
    ...
    if (id != UINT64_MAX) {
        archive_map_add(archive_ctx.map, 0, get_time_in_us(), record->size, record->sid, record->name, archive_path(&archive_ctx, id));
    }
    archive_record_free(record);
...
static void *flush_archive_record(struct ctrl_event *event) {
    ...
    if (!record_to_archive_list) return NULL;

    while (1) {
        if (rte_ring_dequeue(record_to_archive_list, (void **)&record) != 0) break;

        flush_one_archive_record(record);

        nr_flushed += 1;
        if (nr_flushed >= ARCHIVE_RING_SIZE) break;
...
void archive_submit(const char *name, int sid, void *data, size_t data_size, void *parser, size_t parser_size) {
    struct archive_record *record;
...
void archive_init(void) {
    ...
    if (!record_free_list || !record_to_archive_list) return;
...
static int ctx_id_init(struct archive_ctx *ctx) {
    ...
    while (1) {
        ...
        if (strncmp(de->d_name, ctx->prefix, ctx->prefix_len) != 0) continue;

        if (sscanf(de->d_name + ctx->prefix_len, "%lu", &id) != 1) continue;

        if (id > ctx->upper_id) ctx->upper_id = id;
...
static int delete_old_archives(struct archive_ctx *ctx) {
    ...
    if (ctx->nr_id < ctx->nr_to_keep) return 0;

    for (i = 0; i < ctx->nr_id; i++) {
        /* it's sorted */
        if (ctx->ids[i] > ctx->lower_id) break;

        tpa_snprintf(path, sizeof(path), "%s/%s%lu", ctx->dir_path, ctx->prefix, ctx->ids[i]);
    ...
    if (ctx->nr_id) memmove(ctx->ids, &ctx->ids[nr_deleted], ctx->nr_id * sizeof(uint64_t));
...
struct archive_map *map_archive_map_file(const char *path) {
    ...
    mem_file = do_mem_file_create(path, sizeof(struct archive_map), NULL, MEM_FILE_NO_UNLINK, 0);
    if (!mem_file) return NULL;

    return mem_file_data(mem_file);
}

int archive_ctx_init(struct archive_ctx *ctx, const char *dir_path, const char *prefix, int nr_to_keep) {
    DIR *dir;
    ...
    LOG_DEBUG("archive ctx: dir=%s prefix=%s nr_id=%d upper_id=%lu", dir_path, prefix, ctx->nr_id, ctx->upper_id);
...
uint64_t archive_raw(struct archive_ctx *ctx, const void *addr, size_t size) {
    ...
    if (delete_old_archives(ctx) < 0) return UINT64_MAX;

    path = archive_path(ctx, ctx->upper_id + 1);
    fd = open(path, O_WRONLY | O_CREAT | O_NONBLOCK, 0644);
    if (fd < 0) {
        LOG_WARN("failed to open archive file %s for write: %s", path, strerror(errno));
        return UINT64_MAX;
    }

    if (write(fd, addr, size) != size) {
        LOG_WARN("failed to write archive file %s: %s", path, strerror(errno));
        close(fd);
        return UINT64_MAX;
    }
