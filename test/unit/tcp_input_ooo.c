...
static void test_tcp_input_ooo_basic(void) {
    ...
    pkts[0] = ut_inject_data_packet(tsock, tsock->rcv_nxt + off, 1000);
    off += 1000;
    pkts[1] = ut_inject_data_packet(tsock, tsock->rcv_nxt + off, 1000);
    off += 1000;
    pkts[2] = ut_inject_data_packet(tsock, tsock->rcv_nxt + off, 1000);
    off += 1000;
    pkts[3] = ut_inject_data_packet(tsock, tsock->rcv_nxt + off, 1000);
    off += 1000;
...
static void test_tcp_input_ooo_stress(void) {
    ...
    while (seq_lt(seq, end)) {
        len = rand() % 1000 + 1;
        if (seq_gt(seq + len, end)) len = end - seq;
        ...
        ut_tcp_input_one(tsock, pkt);
        {
            ...
            if (p) assert(seq_lt(seq, TCP_SEG(p)->seq));
...
static void test_tcp_input_ooo_stress_harder(void) {
    ...
    WHILE_NOT_TIME_UP() {
        ...
        if ((rand() % 100) <= 3) tsock_drop_ooo_mbufs(tsock);
