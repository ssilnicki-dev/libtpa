...
struct mem_file *do_mem_file_create(const char *path, size_t size, const char *parser, int flags, size_t limit) {
     ...
     */
    if (!(flags & MEM_FILE_NO_UNLINK)) unlink(path);
    ...
    hdr = mmap(next_map_addr, mem_file_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, fd, 0);
    if ((char *)hdr == next_map_addr) {
        if (flags & MEM_FILE_EXPANDABLE) {
            limit += sizeof(struct mem_file_hdr) + parser_size;
            if (limit < mem_file_size) limit = mem_file_size;
        } else {
            limit = mem_file_size;
        }
...
struct mem_file *mem_file_create_expandable(const char *path, size_t size, const char *parser, size_t limit) {
...
int mem_file_expand(struct mem_file *mem_file, size_t size) {
    ...
    if (!(mem_file->flags & MEM_FILE_EXPANDABLE)) {
        LOG_ERR("failed to expand %s to size %lu: not expendable", mem_file->path, new_size);
        return -1;
    }

    if (new_size > mem_file->limit) {
        LOG_ERR("failed to expand %s to size %lu: up to the size limit %lu", mem_file->path, new_size, mem_file->limit);
        return -1;
    }
    ...
    if (err) {
        LOG_ERR("failed to expand %s to size %lu: fallocate failed: %s", mem_file->path, new_size, strerror(err));
        return -1;
    }

    addr = mem_file_remap(mem_file, new_size);
    if (addr == MAP_FAILED) {
        LOG_ERR("failed to expand %s to size %lu: remap failed: %s", mem_file->path, new_size, strerror(errno));
        return -1;
    }
...
void *mem_file_construct(void *data, size_t data_size, void *parser, size_t parser_size, size_t *total_size) {
    struct mem_file_hdr *hdr;
    ...
    if (data) memcpy((char *)hdr + hdr->data_offset, data, data_size);
    if (parser) memcpy((char *)hdr + hdr->parser_offset, parser, parser_size);
...
void *mem_file_map(const char *path, size_t *size, int flags) {
    ...
    if (hdr->magic != MEM_FILE_MAGIC) {
        LOG_ERR("mem_file magic mismatch: expecting 0x%lx while getting 0x%lx", MEM_FILE_MAGIC, hdr->magic);
        goto err;
    }

    if (hdr->size > file_size) {
        LOG_ERR("mem_file size (%ld) is great than real file size (%zd)", hdr->size, file_size);
        goto err;
    }

    if (hdr->data_offset >= file_size) {
        LOG_ERR("mem_file data offset(%ld) is beyond file size(%zd)", hdr->data_offset, file_size);
        goto err;
    }

    if (hdr->parser_offset > file_size) {
        LOG_ERR("mem_file parser offset(%ld) is beyond file size(%zd)", hdr->parser_offset, file_size);
        goto err;
    }
