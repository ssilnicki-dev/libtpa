...
static void handle_link_change(const char *name, int state) {
    ...
    if (!port || state == port->state) return;

    port->state = state;

    LOG_WARN("handle port %hu: %s/%s link %s\n", port->port_id, port->name, port->device_id, state == PORT_LINK_UP ? "up" : "down");
}
...
static int is_bond_slave_backup(const char *slave) {
    ...
    tpa_snprintf(path, PATH_MAX, "/sys/class/net/%s/bonding_slave/state", slave);
    file = fopen(path, "r");
    if (!file) return 0;

    if (fgets(state, sizeof(state), file)) {
        if (!strncmp(state, "backup", 6)) backup = 1;
    }
...
static int bonding_init(void) {
    ...
    if (strlen(bonding_info) == 0) return -1;
...
static int dev_port_init(void) {
    ...
    if (dev.nr_port == 2) {
        ...
        if (bonding_init() < 0) return -1;
    }

#ifdef WITH_XDP
    if (dev.nr_port > 0 && dev.nic == NIC_TYPE_AF_XDP && xdp_ctrl_init() < 0) return -1;
#endif
