...
const char *tpa_id_get(void) {
    ...
    if (getenv("TPA_ID")) return getenv("TPA_ID");
...
static const char *tpa_root_prefix_get(void) {
    ...
    if (prefix == NULL) prefix = "/var/run/tpa";
...
const char *tpa_root_get(void) {
    ...
    if (tpa_root[0]) return tpa_root;

    tpa_snprintf(tpa_root, sizeof(tpa_root), "%s/%s", tpa_root_prefix_get(), tpa_id_get());
...
const char *tpa_log_root_get(void) {
    ...
    if (tpa_log_root[0]) return tpa_log_root;

    prefix = getenv("TPA_LOG_ROOT_PREFIX");
    if (prefix == NULL) prefix = "/var/log/tpa";
...
static int tpa_lock(void) {
    ...
    if (id) {
        ret = do_tpa_lock(id);
        if (ret < 0) fprintf(stderr, "The app with TPA_ID=%s is already running\n", id);
     ...
     * program name.
     */
    if (do_tpa_lock(__progname) == 0) return 0;

    while (1) {
        tpa_snprintf(name, sizeof(name), "%s%d", __progname, ++i);
        if (do_tpa_lock(name) == 0) break;
    }
...
int tpa_init(int nr_worker) {
    ...
    if (tpa_lock() == -1) return -1;

    if (nr_worker <= 0) return -1;
    ...
    if (early_init() == -1) return -1;

    if (ctrl_init() == -1) return -1;

    shell_init();

    dpdk_init(nr_worker);
    if (worker_init(nr_worker) < 0) return -1;

    if (net_dev_init() == -1) return -1;
...
int tpa_extmem_register(void *virt_addr, size_t len, uint64_t *phys_addrs, int nr_page, size_t page_size) {
    uint16_t port;
    int err;
    ...
    err = rte_extmem_register(virt_addr, len, (rte_iova_t *)phys_addrs, nr_page, page_size);
    if (err) {
        LOG_ERR("failed to register extmem: va=%p len=%zd page_size=%zd nr_page=%d", virt_addr, len, page_size, nr_page);
        return err;
    }
    ...
    LOG("registered extmem: va=%p len=%zd page_size=%zd nr_page=%d", virt_addr, len, page_size, nr_page);
...
int tpa_extmem_unregister(void *virt_addr, size_t len) {
    ...
    RTE_ETH_FOREACH_DEV(port) {
        ...
        if (err) LOG_ERR("failed to unmap DMA for port %hu: va=%p len=%zd", port, virt_addr, len);
    }
...
static void append_one_memseg(struct get_memsegs_ctx *ctx, void *virt_addr, size_t size, uint32_t page_size) {
...
static int get_one_memseg(const struct rte_memseg_list *msl, const struct rte_memseg *ms, size_t len, void *ctx) {
    if (!msl->external) append_one_memseg(ctx, msl->base_va, len, msl->page_sz);
...
struct tpa_memseg *tpa_memsegs_get(void) {
    ...
    if (ctx.idx > 0) append_one_memseg(&ctx, NULL, 0, 0);
...
static int cmd_mem_stats(struct shell_cmd_info *cmd) {
    ...
    if (cmd->argc == 1 && strcmp(cmd->argv[0], "-v") == 0) verbose = 1;
