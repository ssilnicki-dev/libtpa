...
void setup_tpa_iovec(struct tpa_iovec *iov, size_t size, int zerocopy) {
    ...
    if (zerocopy) {
        ...
        iov->iov_phys = 1;
        if (zerocopy == TEST_ZCOPY_FALLBACK) iov->iov_phys = 0;
    }
...
ssize_t ut_write(struct tcp_sock *tsock, size_t size) {
    ...
    if (ret < 0) iov.iov_write_done(iov.iov_base, iov.iov_param);
...
void ut_dump_tsock_stats(struct tcp_sock *tsock) {
    ...
    for (i = 0; i < STATS_MAX; i++) {
        if (tsock->stats_base[i]) printf("\t%-32s: %lu\n", stats_name(i), tsock->stats_base[i]);
    }
}
...
/* TODO: remove dupcode */
struct packet *ut_inject_tcp_pkt(struct tcp_sock *tsock, uint32_t seq, uint32_t ack, uint16_t flags, uint16_t win, int payload_len,
    ...
    if (opts->has_ts > 0) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_TS_KIND, tsock->snd_ts);

    if (opts->has_mss > 0) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_MSS_KIND, 1400);

    if (opts->has_wscale > 0) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_WSCALE_KIND, 10);

    if (opt_len) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_EOL_KIND, 0);
...
struct packet *ut_make_input_pkt_chain(struct tcp_sock *tsock, int nr_pkt, int *pkt_size) {
    ...
    for (i = 0; i < nr_pkt; i++) {
        ...
        if (i > 0) packet_chain(pkts[0], pkts[i]);
    }
...
void ut_make_input_pkt_bulk_with_seq(struct tcp_sock *tsock, struct packet **pkts, int nr_pkt, int *pkt_size, uint32_t seq) {
    uint32_t off = 0;
    int i;
...
void ut_make_input_pkt_bulk_randomly_with_seq(struct tcp_sock *tsock, struct packet **pkts, int nr_pkt, uint32_t seq) {
    int pkt_size[nr_pkt];
    int i;
...
void ut_make_input_pkt_bulk(struct tcp_sock *tsock, struct packet **pkts, int nr_pkt, int *pkt_size) {
...
struct packet *ut_inject_ack_packet(struct tcp_sock *tsock, uint32_t ack) {
    ...
    if (tsock->ts_ok) opt_len = ut_tcp_set_opt(ut_packet_tcp_hdr(pkt), 0, TCP_OPT_TS_KIND, tsock->snd_ts);
    ut_ip_set_hdr(pkt, opt_len, 0);

    return pkt;
}

struct packet *ut_inject_sack_packet(struct tcp_sock *tsock, uint32_t ack, struct tcp_sack_block *blocks, int nr_sack) {
    struct rte_tcp_hdr *tcp;
    struct packet *pkt;
    int opt_len = 0;
    ...
    ut_tcp_set_hdr(pkt, tsock->rcv_nxt, ack, TCP_FLAG_ACK, tsock->snd_wnd >> tsock->snd_wscale);
    if (tsock->ts_ok) opt_len = ut_tcp_set_opt(tcp, 0, TCP_OPT_TS_KIND, tsock->snd_ts);
...
struct packet *make_synack_packet(struct tcp_sock *tsock, int has_ts, int mss, int wscale, int sack) {
    ...
    if (has_ts) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_TS_KIND, tsock->snd_ts);
    if (mss) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_MSS_KIND, mss);
    if (wscale) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_WSCALE_KIND, wscale);
    if (sack) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_SACK_PERM_KIND, 0);
    if (opt_len) opt_len = ut_tcp_set_opt(tcp, opt_len, TCP_OPT_EOL_KIND, 0);
...
int ut_timer_process(void) {
    if (ut_with_timer) return timer_process(&worker->timer_ctrl, worker->ts_us);
...
void ut_ndp_handle_reply(struct packet *pkt) {
    ...
    ndp_handle_reply(rte_pktmbuf_mtod_offset(&pkt->mbuf, uint8_t *, off), pkt->mbuf.pkt_len - off);
...
static void assert_on_mbuf_chain(struct rte_mbuf *mbuf) {
    ...
    while (mbuf) {
        if (mbuf->data_off != 0) {
            ...
            char *orig_addr = (char *)mbuf + sizeof(struct rte_mbuf) + rte_pktmbuf_priv_size(mbuf->pool);
            assert((char *)(mbuf->buf_addr) == orig_addr);
        }
        nr_seg_detected += 1;
        pkt_len_detected += mbuf->data_len;

        if (nr_seg_detected >= 2) assert(mbuf->pkt_len == mbuf->data_len);
...
static uint16_t do_ut_tcp_output(struct packet **pkts, uint16_t count, int skip_csum_verify) {
    ...
    for (i = 0; i < nr_pkt; i++) {
        ...
        if (pkt_type == RTE_ETHER_TYPE_ARP || pkt_type == IPPROTO_ICMPV6) goto next;

        if (skip_csum_verify) pkt->mbuf.ol_flags |= PKT_RX_IP_CKSUM_GOOD | PKT_RX_L4_CKSUM_GOOD;
        if (pkt->mbuf.ol_flags & PKT_TX_IP_CKSUM) pkt->mbuf.ol_flags |= PKT_RX_IP_CKSUM_GOOD;
        if (pkt->mbuf.ol_flags & PKT_TX_TCP_CKSUM) pkt->mbuf.ol_flags |= PKT_RX_L4_CKSUM_GOOD;
         ...
         * back again here.
         */
        if (orig_len) TCP_SEG(pkt)->len = orig_len;
...
struct packet *ut_drain_send_buff_at_close(struct tcp_sock *tsock) {
    ...
    while (1) {
        ...
        if (TCP_SEG(pkts[nr_pkt - 1])->flags & TCP_FLAG_FIN) {
            if (nr_pkt >= 2) packet_free_batch(pkts, nr_pkt - 1);
        ...
        /* never ack FIN here */
        ack = tsock->snd_nxt;
        if (tsock->flags & TSOCK_FLAG_FIN_SENT) ack -= 1;
...
void ut_close(struct tcp_sock *tsock, int close_type) {
    ...
    if (close_type == CLOSE_TYPE_CLOSE_DIRECTLY) {
        assert(tsock->state == TCP_STATE_LISTEN || tsock->state == TCP_STATE_SYN_SENT || tsock->state == TCP_STATE_CLOSED);
...
closed:
    if (!ut_test_opts.silent) ut_dump_tsock_stats(tsock);
    ut_assert_mbuf_count();
}
...
static void usage(char *cmd) {
    fprintf(stderr,
            "%s [-m mss] [-w wscale] [-t] [-C 0|1] [-M message size] "
            "   [-T 0|1] [-Z 0|1]\n",
            cmd);
    exit(1);
}
...
uint64_t get_rss_size_in_mb(void) {
    ...
    assert(f != NULL);
    assert(fscanf(f, "%lu %lu %lu %lu %lu %lu %lu", &data[0], &data[1], &data[2], &data[3], &data[4], &data[5], &data[6]) == 7);
...
void ut_exit(void) {
    ...
    printf("rss diff: %d - %d = %d; nr_malloc=%d\n", rss_after, ut_rss_before_testing, rss_after - ut_rss_before_testing,
...
static void ut_dpdk_port_init(void) {
    ...
    dev.caps = TX_OFFLOAD_IPV4_CKSUM | TX_OFFLOAD_TCP_CKSUM | TX_OFFLOAD_TSO | TX_OFFLOAD_MULTI_SEG | EXTERNAL_MEM_REGISTRATION;
...
void ut_init(int argc, char **argv) {
    ...
    if (strstr(argv[0], "tcp_connect_crr") || strstr(argv[0], "arp")) mem_size = 256;
    ...
    tpa_snprintf(dpdk_params, sizeof(dpdk_params), "--no-huge -m %d --no-pci %s", mem_size,
                 getenv("TPA_LOG_DISABLE") ? "--log-level 0" : "");

    /* hardcode some key cfgs to overwrite the system one (if exist) */
    tpa_snprintf(cfg, sizeof(cfg),
                 "net { ip = %s; mask = %s; gw = %s; "
                 "ip6 = %s/%d; gw6 = %s; } "
                 ...
                 "%s",
                 CLIENT_IP_STR, IP_MASK_STR, GW_IP_STR, CLIENT_IP6_STR, 64, GW6_IP_STR, dpdk_params, nr_sock, ut_test_opts.with_sock_trace,
                 ut_port_min, ut_port_max, getenv("TPA_CFG"));
    ...
    tpa_snprintf(cmd, sizeof(cmd), "rm -f %s/%s/socks; rm -rf %s/%s/trace", ut_root_prefix, ut_id, ut_root_prefix, ut_id);
    system(cmd);
