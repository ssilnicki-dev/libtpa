...
static inline int tpa_ip_is_loopback(struct tpa_ip *ip) {
    if (tpa_ip_is_ipv4(ip)) return tpa_ip_get_ipv4(ip) == 0x100007f;

    return ip->u64[0] == 0 && ip->u64[1] == 0x100000000000000ull;
}

static inline int in_same_subnet(struct tpa_ip *local, struct tpa_ip *remote, int prefixlen) {
    ...
    return rte_cpu_to_be_64(a[0]) >> (64 - prefixlen) == rte_cpu_to_be_64(b[0]) >> (64 - prefixlen);
}
