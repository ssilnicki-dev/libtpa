...
static int ndp_solicit(struct tpa_ip *ip, struct tpa_worker *worker) {
    ...
    if (!hdr) return -ERR_PKT_PREPEND_HDR;
...
int ndp_handle_reply(uint8_t *packet, size_t len) {
    ...
    if (len < sizeof(*na) + sizeof(struct nd_opt_hdr) + RTE_ETHER_ADDR_LEN) return 0;

    if (na->nd_na_hdr.icmp6_type == ND_NEIGHBOR_ADVERT && na->nd_na_hdr.icmp6_code == 0) {
...
static void ndp_cache_init(void) {
    ...
    if (getenv("TPA_NDP_SKIP_CACHE_INIT")) return;
    ...
    while (fgets(buf, sizeof(buf), f)) {
        if (sscanf(buf, "%s dev %s lladdr %hhx:%hhx:%hhx:%hhx:%hhx:%hhx", ip6, eth, &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]) !=
            8) {
            continue;
        }

        if (strlen(dev.name) && strcmp(eth, dev.name)) continue;
