...
static inline void tsock_event_add(struct tcp_sock *tsock, uint32_t events) {
    ...
    event->events |= events;
    if (tsock->event.events & tsock->interested_events) flex_fifo_push_if_not_exist(tsock->worker->event_queue, &tsock->event_node);
}

static inline void output_tsock_enqueue(struct tpa_worker *worker, struct tcp_sock *tsock) {
...
void free_err_pkt(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *packet, int err);
