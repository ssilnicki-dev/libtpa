...
#define OFFLOAD_SET(p, field, val)                                                                                                         \
    do {                                                                                                                                   \
        (p)->has_##field = 1;                                                                                                              \
        (p)->field = val;                                                                                                                  \
    } while (0)
...
static void dump_flow_attr(const struct rte_flow_attr *attr) {
    LOG_DEBUG("attributes: ingress=%d, egress=%d, prio=%d, group=%d, transfer=%d", attr->ingress, attr->egress, attr->priority, attr->group,
              attr->transfer);
}
    ...
    if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
        ...
        if (ipv4_spec) {
            LOG_DEBUG("  spec: tos=0x%hhx ttl=%hhx proto=%hhu "
                      ...
                      ipv4_spec->hdr.type_of_service, ipv4_spec->hdr.time_to_live, ipv4_spec->hdr.next_proto_id,
                      IP_ARGS(ipv4_spec->hdr.src_addr), IP_ARGS(ipv4_spec->hdr.dst_addr));
        } else {
            LOG_DEBUG("  spec = null");
        }
        if (ipv4_mask) {
            LOG_DEBUG("  mask: tos=0x%hhx ttl=0x%hhx proto=0x%hhx "
                      "src=" IP_FMT ", dst=" IP_FMT,
                      ipv4_mask->hdr.type_of_service, ipv4_mask->hdr.time_to_live, ipv4_mask->hdr.next_proto_id,
                      IP_ARGS(ipv4_mask->hdr.src_addr), IP_ARGS(ipv4_mask->hdr.dst_addr));
        } else {
            LOG_DEBUG("  mask = null");
        }
        ...
        if (ip6_spec) {
            ...
            LOG_DEBUG("  spec: vtc_flow=%u proto=%hhu src=%s, dst=%s ", ip6_spec->hdr.vtc_flow, ip6_spec->hdr.proto, src, dst);
        } else {
            LOG_DEBUG("  spec = null");
        }
        if (ip6_mask) {
            ...
            LOG_DEBUG("  mask: vtc_flow=%u proto=%hhu src=%s, dst=%s ", ip6_mask->hdr.vtc_flow, ip6_mask->hdr.proto, src, dst);
        } else {
            LOG_DEBUG("  mask = null");
        }
        ...
        if (udp_spec) {
            LOG_DEBUG("  spec: src_port=%hu dst_port=%hu", ntohs(udp_spec->hdr.src_port), ntohs(udp_spec->hdr.dst_port));
        } else {
            LOG_DEBUG("  spec = null");
        }
        if (udp_mask) {
            LOG_DEBUG("  mask: src_port=0x%hx dst_port=0x%hx", ntohs(udp_mask->hdr.src_port), ntohs(udp_mask->hdr.dst_port));
        } else {
            LOG_DEBUG("  mask = null");
        }
        ...
        if (tcp_spec) {
            LOG_DEBUG("  spec: src_port=%hu dst_port=%hu", ntohs(tcp_spec->hdr.src_port), ntohs(tcp_spec->hdr.dst_port));
        } else {
            LOG_DEBUG("  spec = null");
        }
        if (tcp_mask) {
            LOG_DEBUG("  mask: src_port=0x%hx dst_port=0x%hx", ntohs(tcp_mask->hdr.src_port), ntohs(tcp_mask->hdr.dst_port));
        } else {
            LOG_DEBUG("  mask = null");
        }
...
static void add_flow_pattern(struct flow_patterns *patterns, enum rte_flow_item_type type, const void *spec, const void *mask) {
...
static void add_flow_action(struct flow_actions *actions, enum rte_flow_action_type type, const void *conf) {
    int cnt = actions->cnt;
...
static inline void add_rss_action(struct offload_ctx *ctx, uint64_t rss_types) {
    ...
    ctx->rss = (struct rte_flow_action_rss){.func = RTE_ETH_HASH_FUNCTION_DEFAULT,
                                            .level = 0,
                                            .types = rss_types,
...
static void offload_translate(struct offload_rule *rule, struct offload_ctx *ctx) {
    ...
    if (rule->has_mark) add_mark_action(ctx, rule->mark);
...
static struct rte_flow *flow_create(struct offload_ctx *ctx, int port) {
    ...
    if (!flow) {
        LOG_ERR("failed to create rte flow: %d, %s on port %d", ctx->error.type, ctx->error.message, port);
    }
...
static void iavf_offload_workaround_rss(int port, int is_ipv6) {
     ...
     */
    if (!flows[idx]) flows[idx] = (void *)(uintptr_t)0xff;
...
static int offload_destroy(struct offload_list *list) {
    ...
    while (offload) {
        ...
        if (ret != 0) {
            LOG("failed to destroy sub-offload %s, %s on port %d", list->name, error.message, offload->port);
            failed = 1;
        }
...
static int offload_create(struct offload_list *list, struct offload_rule *rule, int priority) {
    ...
    if (dev.nic == NIC_TYPE_IAVF) iavf_offload_workaround(&ctx);

    for (i = 0; i < dev.nr_port; i++) {
        offload = malloc(sizeof(struct offload));
        if (!offload) goto fail;
...
static int tsock_offload_do_create(struct tcp_sock *tsock, int ipv6) {
    ...
    if (tsock->state == TCP_STATE_LISTEN) priority = 1;
    ...
    if (offload_cfg.enable_flow_mark) OFFLOAD_SET(&rule, mark, make_flow_mark(tsock->worker->id, tsock->sid));
...
int port_block_offload_create(struct port_block *block) {
    ...
    if (!offload_cfg.enable_port_block_offload) return 0;

    offload_set_name(&block->offload_list, "port block %hu-%hu", block->start, block->end);
    ...
    if (dev.ip4) tpa_ip_set_ipv4(&local_ips[nr_ip++], dev.ip4);
    if (!is_ip6_any(&dev.ip6.ip)) local_ips[nr_ip++] = dev.ip6.ip;

    for (i = 0; i < nr_ip; i++) {
        OFFLOAD_SET(&rule, dst_ip, local_ips[i]);
        if (offload_create(&block->offload_list, &rule, 1) < 0) goto fail;
    }
