...
static void assert_archive_file_count(const char *prefix, int count) {
    ...
    tpa_snprintf(cmd, sizeof(cmd), "[ $(ls %s/%s[0-9]* | wc -l) -eq %d ]", tpa_log_root_get(), prefix, count);

    assert(system(cmd) == 0);
}

static void test_archive(const char *prefix, uint32_t data_size, int init_count, int default_count) {
