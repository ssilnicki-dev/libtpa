...
static inline void timer_stop(struct timer *timer) {
    if (timer_is_stopped(timer)) return;
...
static inline void timer_start(struct timer *timer, uint64_t now, uint64_t expire) {
    ...
    if (slot_off > NR_SLOT) slot_off = NR_SLOT;
    slot_idx = slot_off + timer_ctrl->curr_slot_idx;

    if (!timer_is_stopped(timer) && (slot_idx & SLOT_MASK) == timer->slot_idx) return;
...
static inline void timer_init(struct timer *timer, struct timer_ctrl *timer_ctrl, void (*cb)(struct timer *timer), void *arg,
                              uint64_t now) {
