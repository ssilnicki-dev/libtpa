...
static void process_conn(struct connection *conn) {
    ...
    if (events & (TPA_EVENT_IN | TPA_EVENT_ERR | TPA_EVENT_HUP)) ret = conn_on_read(conn);

    if (ret >= 0 && (events & (TPA_EVENT_OUT | TPA_EVENT_ERR | TPA_EVENT_HUP))) ret = conn_on_write(conn);

    if (ret < 0 || (events & (TPA_EVENT_ERR | TPA_EVENT_HUP)) || conn->to_close) conn_close(conn);
}
