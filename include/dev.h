...
static inline void dev_port_txq_flush(uint16_t port_id, uint16_t queue_id) {
    ...
    if (txq->nr_pkt == 0 || tpa_cfg.nr_dpdk_port == 0) return;
    ...
    if (unlikely(dev.ports[port_id].state != PORT_LINK_UP)) port_id = dev_port_id_get();

    count = rte_eth_tx_burst(port_id, queue_id, (struct rte_mbuf **)txq->pkts, count);

    txq->nr_pkt -= count;
    if (txq->nr_pkt) memmove(&txq->pkts[0], &txq->pkts[count], txq->nr_pkt * sizeof(struct packet *));
}
...
static inline int dev_port_txq_enqueue(uint16_t port_id, uint16_t queue_id, struct packet *pkt) {
    ...
    if (txq->nr_pkt >= TXQ_BUF_SIZE) return -ERR_DEV_TXQ_FULL;

    txq->pkts[txq->nr_pkt++] = pkt;

    pktfuzz(txq);

    if (txq->nr_pkt >= BATCH_SIZE) dev_port_txq_flush(port_id, queue_id);
