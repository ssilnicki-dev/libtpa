...
static int init_one_worker(struct tpa_worker *worker, uint8_t id) {
    ...
    PANIC_ON(worker->output == NULL || worker->delayed_ack == NULL || worker->event_queue == NULL || worker->accept == NULL ||
    ...
    if (!worker->tx_desc_pool) return -1;
    ...
    if (packet_pool_create(&worker->zwrite_pkt_pool, 25.0 / tpa_cfg.nr_worker, 0, "zwrite-mbuf-mp-%d", worker->id) < 0) return -1;

    return packet_pool_create(&worker->hdr_pkt_pool, 12.5 / tpa_cfg.nr_worker, RTE_PKTMBUF_HEADROOM, "hdr-mbuf-mp-%d", worker->id);
}
...
int worker_init(uint32_t nr_worker) {
    ...
    for (i = 0; i < nr_worker; i++) {
        if (init_one_worker(&workers[i], i) < 0) return -1;
    }
...
static int flush_neigh_queue(struct tpa_worker *worker) {
    if (likely(flex_fifo_count(worker->neigh_flush_queue) == 0)) return 0;
...
static inline void drop_ooo_mbufs(struct tpa_worker *worker) {
    if (unlikely(too_many_used_mbufs(worker))) do_drop_ooo_mbufs(worker);
}
...
static void print_stats(struct shell_buf *reply, uint64_t *stats) {
    ...
    for (i = 0; i < STATS_MAX; i++) {
        if (stats[i]) shell_append_reply(reply, "\t%-32s: %lu\n", stats_name(i), stats[i]);
    }
}
...
static void dump_worker(struct tpa_worker *worker, struct shell_buf *reply, int reset_starvation) {
    ...
    shell_append_reply(reply,
                       ...
                       "\t%-32s: %u\n"
                       "\t%-32s: %u\n",
                       "tid", worker->tid, "cycles.busy", worker->cycles.busy, "cycles.outside_worker", worker->cycles.outside_worker,
                       "cycles.total", worker->cycles.total, "last_run", (double)TSC_TO_US(TS_DIFF(rte_rdtsc(), worker->cycles.end)) / 1e6,
                       "last_poll", (double)TSC_TO_US(TS_DIFF(rte_rdtsc(), worker->cycles.start)) / 1e6, "avg_runtime",
                       _US(vstats_avg(&worker->runtime)), "avg_starvation", _US(vstats_avg(&worker->starvation)), "max_runtime",
                       _US(worker->runtime.max) / 1e3, "max_starvation", _US(worker->starvation.max) / 1e3, "nr_tsock", worker->nr_tsock,
                       "nr_tsock_total", worker->nr_tsock_total, "port_txq.size", TXQ_BUF_SIZE, "nr_ooo_mbuf", worker->nr_ooo_mbuf,
                       "nr_in_process_mbuf", worker->nr_in_process_mbuf, "nr_write_mbuf", worker->nr_write_mbuf);
    ...
    for (i = 0; i < dev.nr_port; i++) {
        ...
        shell_append_reply(reply, "\t%-32s: %u\n", buf, dev_port_rxq(i, worker->queue)->write - dev_port_rxq(i, worker->queue)->read);
    }

    if (reset_starvation) worker->starvation.max = 0;
...
static int cmd_worker(struct shell_cmd_info *cmd) {
    ...
    if (cmd->argc == 1 && strcmp(cmd->argv[0], "--reset-starvation") == 0) reset_starvation = 1;
...
void free_err_pkt(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt, int err) {
    if (err) {
        if (tsock) trace_error(tsock, -err);
