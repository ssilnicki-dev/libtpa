...
static inline const char *desc_flags_to_str(uint8_t flags) {
    ...
    if (flags & TX_DESC_FLAG_MEM_FROM_MBUF) len += snprintf(buf + len, sizeof(buf) - len, "NON-ZWRITE ");

    if (flags & TX_DESC_FLAG_MEASURE_LATENCY) len += snprintf(buf + len, sizeof(buf) - len, "LAT ");

    if (flags & TX_DESC_FLAG_RETRANS) len += snprintf(buf + len, sizeof(buf) - len, "RETRANS ");

    if (flags & TX_DESC_FLAG_SACKED) len += snprintf(buf + len, sizeof(buf) - len, "SACKED ");
...
#define str_buf_append(s, ...) (s)->len += snprintf((s)->buf + (s)->len, sizeof((s)->buf) - (s)->len, __VA_ARGS__)

#define str_buf_vsnprintf(s, fmt)                                                                                                          \
    do {                                                                                                                                   \
        va_list ap;                                                                                                                        \
        va_start(ap, fmt);                                                                                                                 \
...
static void dump_trace(struct tsock_trace *trace) {
    ...
    while (ctx.iter < trace->off) {
        ...
        if (dump_ops[record->type](&ctx) < 0) break;
    }
}
...
static void dump_tsock_trace(struct tsock_trace *trace) {
    ...
    trace_printf("%s:%hu %s:%hu worker=%d\n", local_ip, ntohs(trace->local_port), remote_ip, ntohs(trace->remote_port), trace->worker);
...
static int list_tsock_trace(const char *path, struct tsock_trace *trace) {
    ...
    TSOCK_TRACE_FOREACH(trace) {
        ...
        printf("%-42s %-8lu %-6lu %-28s %-6d %s\n", path, off, trace->size, str_time(trace->init_ts_us), trace->sid, name);
    }
...
int main(int argc, char *argv[]) {
    ...
    file = argv[optind++];
    if (optind < argc) off = strtol(argv[optind], NULL, 10);

    if (map_tsock_trace_file(file) < 0) exit(1);

    if (list_only) return list_tsock_trace(file, trace);
