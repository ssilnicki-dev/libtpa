...
static void trace_root_init(void) {
    ...
    while (1) {
        ...
        if (unlinkat(dirfd(dir), de->d_name, 0) < 0) {
            LOG_ERR("failed to remove trace file %s/%s: %s", path, de->d_name, strerror(errno));
        }
    }
...
static void tsock_trace_ctrl_init(void) {
    ...
    if (tsock_trace_ctrl.file == NULL) return;
...
void tsock_trace_init(struct tcp_sock *tsock, int sid) {
    ...
    if (!tsock_trace_ctrl.file) return;
...
void tsock_trace_archive(struct tsock_trace *trace, const char *fmt, ...) {
    ...
    archive_submit(name, trace->sid, trace, trace->size, tsock_trace_ctrl.parser, tsock_trace_ctrl.parser_size);
}
...
static void convert_sock_opts(const struct tpa_sock_opts *opts, struct tpa_sock_opts_short *short_opts) {
...
static int tsock_init(struct tcp_sock *tsock, int sid, const struct tpa_sock_opts *opts) {
    ...
    if (opts) convert_sock_opts(opts, &tsock->opts);
    ...
    if (!tsock->rxq.objs) return -ENOMEM;
...
int tsock_try_update_eth_hdr(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    if (eth_lookup(worker, &tsock->remote_ip, &eth) < 0) return -1;
...
static int enlarge_sock_count(void) {
    ...
    if (sock_ctrl->expand_failed) return -1;
...
again:
    ...
    if (tsock && tsock_init(tsock, sid, opts) < 0) tsock = NULL;
...
static int resolve_ip(const char *ip_str, struct tpa_ip *ip, int listen_mode) {
    if (!tpa_ip_from_str(ip, ip_str)) goto err;
...
int tpa_connect_to(const char *server, uint16_t port, const struct tpa_sock_opts *opts) {
    ...
    if (resolve_ip(server, &remote_ip, 0) < 0) return -1;

    if ((dev.ip4 == 0 && tpa_ip_is_ipv4(&remote_ip)) || (is_ip6_any(&dev.ip6.ip) && !tpa_ip_is_ipv4(&remote_ip))) {
    ...
    if (tsock_bind(tsock, &remote_ip, port) < 0) goto free_tsock;

    if (tcp_connect(tsock) < 0) goto free_port;
...
static int listen_ip_get(const char *local, struct tpa_ip *local_ip) {
    ...
    if (resolve_ip(local, local_ip, 1) < 0) goto err;

    if (tpa_ip_is_ipv4(local_ip)) {
        if (tpa_ip_get_ipv4(local_ip) != dev.ip4) goto err;
    } else {
        if (!tpa_ip_equal(local_ip, &dev.ip6.ip)) goto err;
    }
...
int tpa_listen_on(const char *local, uint16_t port, const struct tpa_sock_opts *opts) {
    ...
    if (listen_ip_get(local, &local_ip) < 0) return -1;
...
int tpa_accept_burst(struct tpa_worker *worker, int *sid, int nr_sid) {
    ...
    while (nr_valid_sock < nr_sid) {
        ...
        if (tsock->sid < 0) continue;

        if (tsock->state != TCP_STATE_ESTABLISHED && tsock->state != TCP_STATE_CLOSE_WAIT) continue;
...
ssize_t tpa_zwritev(int sid, const struct tpa_iovec *iov, int nr_iov) {
    ...
    if (unlikely(ret < 0 && errno == EAGAIN)) TSOCK_STATS_INC(tsock, WRITE_EAGAIN);
...
int listen_tsock_lookup(struct packet *pkt, struct tcp_sock **tsock_ptr) {
    ...
    if (!tsock) return -ERR_NO_SOCK;

    debug_assert(tsock->state == TCP_STATE_LISTEN);
    if (!tuple_matches(tsock, pkt)) return -ERR_NO_SOCK;
...
int tsock_lookup_slowpath(struct tpa_worker *worker, struct packet *pkt, struct tcp_sock **tsock_ptr) {
    struct sock_key key;
    struct tcp_sock *tsock;
    ...
    if (tsock->sid < 0) return -WARN_STALE_PKT_TUPLE_MISMATCH;
...
int sock_init_early(void) {
    ...
    mem_file = mem_file_create_expandable(path, size, "sock-list", sizeof(struct tcp_sock) * (10ul << 20));
...
static void set_drop_ooo_threshold(void) {
    ...
    if (tcp_cfg.drop_ooo_threshold) return;

    tcp_cfg.drop_ooo_threshold = (pool->size / tpa_cfg.nr_worker - NR_RX_DESC * tpa_cfg.nr_dpdk_port) * 8 / 10;
}
...
#define REGULATE_TCP_CFG(option)                                                                                                           \
    do {                                                                                                                                   \
        if (dev.option && tcp_cfg.option > dev.option) {                                                                                   \
            LOG("%s (%u) is beyond the dev cap (%u); clamp it", #option, tcp_cfg.option, dev.option);                                      \
...
static int tcp_cfg_set_with_regulation(struct cfg_spec *spec, const char *val) {
    if (cfg_spec_set_num(spec, val) < 0) return -1;
