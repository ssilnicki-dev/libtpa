...
static inline void mbuf_set_offload(struct packet *pkt, struct eth_ip_hdr *net_hdr, struct rte_tcp_hdr *tcp, int is_ipv6,
                                    uint16_t tcp_hdr_len, uint16_t payload_len, uint16_t packet_id, uint16_t snd_mss) {
    struct rte_mbuf *m = &pkt->mbuf;
    ...
    if (!is_ipv6) {
        ...
        if (likely(dev.caps & TX_OFFLOAD_TCP_CKSUM)) {
            ...
            if (unlikely(!(dev.caps & TX_OFFLOAD_PSEUDO_HDR_CKSUM))) tcp->cksum = rte_ipv4_phdr_cksum(&net_hdr->ip4, m->ol_flags);
        } else {
            tcp->cksum = 0;
        ...
        if (likely(dev.caps & TX_OFFLOAD_TCP_CKSUM)) {
            ...
            if (unlikely(!(dev.caps & TX_OFFLOAD_PSEUDO_HDR_CKSUM))) tcp->cksum = rte_ipv6_phdr_cksum(&net_hdr->ip6, m->ol_flags);
        } else {
            tcp->cksum = 0;
...
static inline int calc_tcp_opt_len(struct tcp_sock *tsock, uint16_t flags, uint32_t *enabled_opts) {
    uint32_t opts = 0;
    uint16_t len = 0;
...
static __rte_noinline void fill_uncommon_opts(const struct tcp_sock *tsock, uint32_t opts, uint8_t *addr) {
    struct tcp_opt *opt;
...
static inline void fill_opts(const struct tcp_sock *tsock, uint32_t opts, uint8_t *addr) {
    ...
    if (unlikely(opts & ~TCP_OPT_TS_BIT)) fill_uncommon_opts(tsock, opts, addr);
}

static inline int prepend_tcp_hdr(struct tcp_sock *tsock, struct packet *pkt, uint32_t seq, uint8_t tcp_flags) {
    struct rte_mbuf *m = &pkt->mbuf;
    ...
    if (hdr == NULL) return -ERR_PKT_PREPEND_HDR;
    ...
    if (tsock->rcv_wnd >= TCP_WINDOW_MAX) tsock->rcv_wnd = 0;
    ...
    *hdr = tsock->net_hdr;
    ...
    if (opts & TCP_OPT_SACK_BIT) snd_mss -= TCP_OPT_SACK_SPACE(tsock->nr_sack_block);

    mbuf_set_offload(pkt, hdr, tcp, tsock->is_ipv6, tcp_hdr_len, payload_len, tsock->packet_id++, snd_mss);

    return 0;
}

static inline void update_tcp_csum_offload(struct packet *pkt, struct eth_ip_hdr *net_hdr, struct rte_tcp_hdr *tcp, int is_ipv6,
                                           uint16_t snd_mss) {
    struct rte_mbuf *m = &pkt->mbuf;
...
int xmit_rst_for_listen(struct tpa_worker *worker, struct tcp_sock *tsock, struct packet *pkt) {
    ...
    if (!reply_pkt) return -ERR_PKT_ALLOC_FAIL;
    ...
    mbuf_set_offload(reply_pkt, net_hdr, tcp, pkt->flags & PKT_FLAG_IS_IPV6, sizeof(struct rte_tcp_hdr), 0, tsock->packet_id++, 0);
...
uint32_t isn_gen(struct tpa_ip *local_ip, struct tpa_ip *remote_ip, uint16_t local_port, uint16_t remote_port) {
    uint32_t seq;

    /* we ignore local ip here as it does not change */
    seq = rte_crc32_u64(remote_ip->u64[0] | (uint64_t)local_port, remote_ip->u64[1] | (uint64_t)remote_port);
...
static void __rte_noinline tcp_zero_wnd_probe(struct tpa_worker *worker, struct tcp_sock *tsock) {
    if (worker->ts_us - last_ts_in_us(tsock, LAST_TS_SND_PKT) < (tsock->rto << tsock->zero_wnd_probe_shift)) return;
 ...
 */
static inline int xmit_one_packet(struct tpa_worker *worker, struct tcp_sock *tsock, struct xmit_ctx *ctx) {
    ...
    while (1) {
        ...
        if (likely(desc->flags & TX_DESC_FLAG_SACKED) == 0) break;
    ...
    if (!hdr_pkt) return -ERR_PKT_ALLOC_FAIL;
    ...
    while (budget > 0) {
        ...
        if (unlikely(desc->flags & TX_DESC_FLAG_SACKED)) break;
        ...
        if (hdr_pkt->mbuf.nb_segs >= tcp_cfg.pkt_max_chain) break;

        if (unlikely(!desc)) break;
    }

    if (unlikely(size == 0)) goto error;
...
static inline int do_tcp_xmit_data(struct tpa_worker *worker, struct tcp_sock *tsock, struct xmit_ctx *ctx) {
    uint32_t nr_to_xmit;
    int size = 0;
    int ret;
    int i;
    ...
    if (likely(size > 0)) {
         ...
         */
        if (timer_is_stopped(&tsock->timer_rto)) tsock_rearm_timer_rto(tsock, worker->ts_us);
...
static uint32_t tcp_xmit_data(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    if (ctx.seq == tsock->snd_nxt) return 0;
...
void tcp_retrans(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    if (seq_lt(tsock->retrans.seq, tsock->snd_una)) tcp_reset_retrans(tsock, tsock->snd_una, tsock->txq.una);
...
void tcp_fast_retrans(struct tpa_worker *worker, struct tcp_sock *tsock, int budget) {
    ...
    if (budget < tsock->snd_mss) budget = tsock->snd_mss;
...
static inline void refire_out_event_if_needed(struct tcp_sock *tsock) {
    if (unlikely(tcp_txq_unfinished_pkts(&tsock->txq) == 0)) tsock_event_add(tsock, TPA_EVENT_OUT);
}
...
static void write_revoke(struct tcp_sock *tsock, struct write_ctx *ctx) {
    ...
    for (i = 0; i < ctx->nr_desc; i++) {
        ...
        if (desc->flags & TX_DESC_FLAG_MEM_FROM_MBUF) packet_free(desc->pkt);

        tx_desc_free(tsock->worker->tx_desc_pool, desc);
    }
}

static int write_one_iov(struct tcp_sock *tsock, const struct tpa_iovec *iov, struct write_ctx *ctx) {
    ...
    do {
        if (unlikely(ctx->nr_desc + 1 > ctx->nr_desc_free)) return -1;
...
static __rte_noinline ssize_t tsock_zwritev_slowpath(struct tcp_sock *tsock, const struct tpa_iovec *iov, int nr_iov,
                                                     struct write_ctx *ctx) {
    int i;

    ctx->nr_fallback = 0;
    for (i = ctx->nr_desc; i < nr_iov; i++) {
        if (write_one_iov(tsock, &iov[i], ctx) < 0) return -1;
    }

    if (unlikely(trace_cfg.more_trace)) tsock_trace_zwritev(tsock, nr_iov, nr_iov, ctx->size, ctx->nr_fallback);
...
#define TSOCK_WRITE_CHECK(tsock)                                                                                                           \
    do {                                                                                                                                   \
...
ssize_t tsock_zwritev(struct tcp_sock *tsock, const struct tpa_iovec *iov, int nr_iov) {
    ...
    if (unlikely(tcp_cfg.measure_latency)) ctx.start_tsc = rte_rdtsc();
    ...
    for (i = 0; i < nr_iov; i++) {
        if (unlikely(iov[i].iov_phys == 0 || iov[i].iov_len == 0 || iov[i].iov_len > tcp_cfg.write_chunk_size)) goto slowpath;

        if (unlikely(ctx.nr_desc + 1 > ctx.nr_desc_free)) goto fail;

        desc = tx_desc_alloc(tsock->worker->tx_desc_pool);
        if (unlikely(!desc)) goto fail;
    ...
    if (unlikely(trace_cfg.more_trace)) tsock_trace_zwritev(tsock, nr_iov, nr_iov, ctx.size, 0);
...
slowpath:
    if (tsock_zwritev_slowpath(tsock, iov, nr_iov, &ctx) >= 0) return ctx.size;
fail:
...
static inline void tsock_init_net_hdr(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    if (eth_lookup(worker, &tsock->remote_ip, &eth) < 0) tsock->flags |= TSOCK_FLAG_MISSING_ARP;
...
uint16_t calc_snd_mss(const struct tcp_sock *tsock, int has_ts, int passive, uint16_t nego_mss) {
    ...
    if (tcp_cfg.usr_snd_mss > 0) {
        ...
        if (snd_mss < 0) snd_mss = TCP_MSS_DEFAULT;
    }

    if (passive == 0) return snd_mss;
...
int xmit_syn(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    tsock->flags |= TSOCK_FLAG_SYN_NEEDED;
    if (tsock->state == TCP_STATE_SYN_RCVD) tsock->flags |= TSOCK_FLAG_ACK_NEEDED;
     ...
     * timeout.
     */
    if (ret == -ERR_DEV_TXQ_FULL) output_tsock_enqueue(worker, tsock);

    if (tsock->rto_shift == 0) timer_start(&tsock->timer_rto, worker->ts_us, tsock->rto);
...
static __rte_noinline int tcp_close(struct tpa_worker *worker, struct tcp_sock *tsock) {
    if (tsock->flags & TSOCK_FLAG_CLOSE_PROCESSED) return tsock->state;
...
static inline int tcp_output_one(struct tpa_worker *worker, struct tcp_sock *tsock) {
    ...
    /* XXX: count it */
    if (unlikely(tsock->sid < 0)) return 0;

    state = tsock->state;
    if (unlikely(tsock->close_issued)) state = tcp_close(worker, tsock);
    ...
    if (tcp_txq_to_send_pkts(&tsock->txq) > 0) {
        ...
        tsock->flags |= TSOCK_FLAG_FIN_NEEDED | TSOCK_FLAG_ACK_NEEDED | TSOCK_FLAG_FIN_SENT;
        if (tsock->rto_shift == 0) {
            timer_start(&tsock->timer_rto, tsock->worker->ts_us, tsock->rto);
        }
    }

    if (tsock_flags_to_tcp_flags(tsock->flags)) xmit_flag_packet(worker, tsock);
...
static inline void tcp_flush_delayed_ack(struct tpa_worker *worker) {
    ...
    for (i = 0; i < BATCH_SIZE; i++) {
        ...
        if ((uint32_t)worker->ts_us - tsock->last_ack_sent_ts < tcp_cfg.delayed_ack) break;

        if (tsock->flags & TSOCK_FLAG_ACK_NEEDED) xmit_flag_packet(worker, tsock);
        flex_fifo_pop(worker->delayed_ack);
    }
}
...
int tcp_output(struct tpa_worker *worker) {
    ...
    for (i = 0; i < nr_tsock; i++) {
        ...
        if (unlikely(err)) WORKER_TSOCK_STATS_INC(worker, tsock, -err);
    }
