...
static const char *state_to_str(int state) {
    ...
    if (state < 0 || state >= (int)(ARRAY_SIZE(states))) return "unknown-state";
...
static const char *event_to_str(uint32_t event) {
    ...
    if (event & TPA_EVENT_IN) len += snprintf(buf + len, sizeof(buf) - len, "IN | ");

    if (event & TPA_EVENT_OUT) len += snprintf(buf + len, sizeof(buf) - len, "OUT | ");

    if (event & TPA_EVENT_ERR) len += snprintf(buf + len, sizeof(buf) - len, "ERR | ");

    if (event & TPA_EVENT_HUP) len += snprintf(buf + len, sizeof(buf) - len, "HUP | ");
...
#define PRINT_JSON_COMMA(x)                                                                                                                \
    do {                                                                                                                                   \
        if (x) printf(",\n");                                                                                                              \
        x = 1;                                                                                                                             \
    } while (0)

#define __SHOW(name, fmt, val, quote)                                                                                                      \
    do {                                                                                                                                   \
        if (show_json) {                                                                                                                   \
...
static void print_stats(uint64_t *stats, int print_field_comma) {
    ...
    for (i = 0; i < STATS_MAX; i++) {
        if (stats[i]) __SHOW(stats_name(i), "%lu", stats[i], 0);
    }
}
...
static char *get_sid(struct tcp_sock *tsock) {
    ...
    if (tsock->sid >= 0) {
        if (tsock->sid != (int)(tsock - sock_ctrl->socks)) fprintf(stderr, "error: invalid sid detected: %d\n", tsock->sid);
...
#define SHOW_LAST_TS(field, idx)                                                                                                           \
    do {                                                                                                                                   \
        if (show_json) {                                                                                                                   \
            if (tsock->last_ts[idx]) SHOW_FIELD2(field, "%.3f", (TS_DIFF(_US(now), _US(tsock->last_ts[idx])) / 1e6));                      \
        } else {                                                                                                                           \
...
#define __SHOW_VSTATS(field, fmt1, avg1, max1, fmt2, unit, avg2, max2)                                                                     \
    do {                                                                                                                                   \
        if (show_json) {                                                                                                                   \
...
#define SHOW_VSTATS(field, u)                                                                                                              \
    __SHOW_VSTATS(field, "%lu", (uint64_t)(vstats_avg(&tsock->field)), (uint64_t)(tsock->field.max), "%lu", u,                             \
                  (uint64_t)(vstats_avg(&tsock->field)), (uint64_t)(tsock->field.max))
#define SHOW_LAT_VSTATS(field)                                                                                                             \
    __SHOW_VSTATS(field, "%.7f", _S(vstats_avg(&tsock->field)), _S(tsock->field.max), "%.1f", "us", _US(vstats_avg(&tsock->field)),        \
                  _US(tsock->field.max))
#define SHOW_SIZE_VSTATS(field)                                                                                                            \
    __SHOW_VSTATS(field, "%lu", vstats_avg(&tsock->field), (uint64_t)(tsock->field.max), "%.3f", "KB",                                     \
                  (double)(vstats_avg(&tsock->field)) / 1024, (double)tsock->field.max / 1024)

static char *get_mac_addr(const struct rte_ether_addr *addr) {
    static char mac[128];

    snprintf(mac, sizeof(mac), "%02x:%02x:%02x:%02x:%02x:%02x", addr->addr_bytes[0], addr->addr_bytes[1], addr->addr_bytes[2],
...
static void dump_sock(struct tcp_sock *tsock, int verbose) {
    ...
    snprintf(connection, sizeof(connection), "%s:%hu%s%s:%hu", local_ip, htons(tsock->local_port), show_json ? "-" : " ", remote_ip,
             htons(tsock->remote_port));
...
static int dump_tsocks(struct tcp_sock *socks) {
    ...
    for (i = 0; i < sock_ctrl->nr_max_sock; i++) {
        ...
        if (tsock->sid == TSOCK_SID_UNALLOCATED) continue;

        if (has_dump_mask && sid_dump_mask[i] == 0) continue;

        if (list_all || sid_dump_mask[i] || tsock->sid >= 0) dump_sock(tsock, verbose);
    }
