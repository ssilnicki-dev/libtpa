...
static void test_flex_fifo_basic(int ring_size) {
    ...
    /* enqueue 3 */
    FLEX_FIFO_NODE_INIT(&ts[0].node);
    flex_fifo_push(ff, &ts[0].node);
    FLEX_FIFO_NODE_INIT(&ts[1].node);
    flex_fifo_push(ff, &ts[1].node);
    FLEX_FIFO_NODE_INIT(&ts[2].node);
    flex_fifo_push(ff, &ts[2].node);
...
static void test_flex_fifo_stress(int ring_size) {
    ...
    WHILE_NOT_TIME_UP() {
        ...
        /* simulate burst */
        if (rand() % 1000 == 1) nr_to_push = rand() % 4096;

        for (i = 0; i < nr_to_push; i++)
            push_ts(ff);

        for (i = 0; i < nr_to_pop; i++) {
            if (pop_and_verify_ts(ff, &id, 1) < 0) break;
        }

        ON_INTERVAL(1000 * 1000) {
            printf("%-4u curr : total=%-8u ring=%-4u list=%-8u   "
                   "total: ring=%-12lu list=%-12lu ring_ratio=%.3f\n",
                   loop++, flex_fifo_count(ff), flex_fifo_ring_count(ff), flex_fifo_list_count(ff), ff->ring_total, ff->list_total,
...
static void test_flex_fifo_stress_with_remove(int ring_size) {
    ...
    WHILE_NOT_TIME_UP() {
        ...
        for (i = 0; i < nr_to_pop; i++) {
            if (pop_and_verify_ts(ff, &id, 0) < 0) break;
        }
