...
static void test_extmem_basic(void) {
    ...
    addr = mmap(NULL, EXTBUF_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    assert(addr != MAP_FAILED);
    memset(addr, 0, EXTBUF_SIZE);

    assert(tpa_extmem_register(addr, EXTBUF_SIZE, NULL, EXTBUF_SIZE / EXTBUF_PGSZ, EXTBUF_PGSZ) == 0);
