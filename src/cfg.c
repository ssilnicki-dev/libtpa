...
enum { TOKEN_NONE, TOKEN_BRACE_LEFT, TOKEN_BRACE_RIGHT, TOKEN_STRING, TOKEN_EQUAL, TOKEN_OPT_END, TOKEN_BAD, TOKEN_CONSUMED };
...
static int match_quoted_string(struct cfg_ctx *ctx) {
    ...
    while (1) {
        ...
        if (token_put_char(ctx, c) < 0) return -1;
    }
...
static int get_string_token(struct cfg_ctx *ctx, char c) {
    ...
    if (!is_valid_char(c)) return TOKEN_BAD;
...
static int token_do_peek_type(struct cfg_ctx *ctx, int count_nl) {
    ...
    if (ctx->token != TOKEN_CONSUMED) return ctx->token;
    ...
    while ((c = fgetc(ctx->file)) != EOF) {
        ...
        case '\n':
            if (count_nl) token = TOKEN_OPT_END;
            break;
        case ';':
            token = TOKEN_OPT_END;
            break;
...
static int cfg_file_match_brace(struct cfg_ctx *ctx, int type) {
    if (token_get_type(ctx) != type) {
        LOG_WARN("cfg: expected '%c', while got '%s'", type == TOKEN_BRACE_LEFT ? '{' : '}', token_get_name(ctx));
        return -1;
    }
...
static int cfg_file_match_equal(struct cfg_ctx *ctx) {
    if (token_get_type(ctx) != TOKEN_EQUAL) {
        LOG_WARN("cfg: opt=%s.%s: expected '=', while got '%s'", ctx->section->name, ctx->opt->name, token_get_name(ctx));
        return -1;
    }
...
static int cfg_file_match_opt_val(struct cfg_ctx *ctx) {
    ...
    while (1) {
        ...
        if (token == TOKEN_OPT_END) break;

        if (token != TOKEN_STRING) {
            LOG_WARN("cfg: opt %s.%s has invalid value: '%s'", ctx->section->name, opt->name, token_get_name(ctx));
            return -1;
        }
...
static int cfg_file_match_options(struct cfg_ctx *ctx) {
    ...
    while (1) {
        /* end of section */
        if (token_peek_type(ctx) == TOKEN_BRACE_RIGHT) break;
        ...
        if (cfg_file_match_opt_name(ctx) < 0 || cfg_file_match_equal(ctx) < 0 || cfg_file_match_opt_val(ctx) < 0) {
            free(opt);
            return -1;
        }
...
static int cfg_file_match_section(struct cfg_ctx *ctx) {
    ...
    if (cfg_file_match_section_name(ctx) < 0 || cfg_file_match_brace(ctx, TOKEN_BRACE_LEFT) < 0 || cfg_file_match_options(ctx) < 0 ||
...
int cfg_file_parse(FILE *f) {
    ...
    while (1) {
        /* end of cfg file */
        if (token_peek_type(&ctx) == TOKEN_NONE) break;

        if (cfg_file_match_section(&ctx) < 0) return -1;
    }
...
const char *cfg_file_opt_get(const char *name) {
    ...
    CFG_SECTION_FOREACH(section) {
        if (strcmp(section->name, section_name)) continue;

        for (opt = section->opts; opt != NULL; opt = opt->next) {
            if (strcmp(opt->name, opt_name) == 0) return opt->val;
        }
    }
...
int cfg_spec_set_num(struct cfg_spec *spec, const char *val) {
    ...
    if ((spec->flags & CFG_FLAG_HAS_MIN) && num < spec->min) {
        LOG_WARN("set %s fail, %s is less than cfg_spec min: %u", spec->name, val, spec->min);
        return -1;
    }

    if ((spec->flags & CFG_FLAG_HAS_MAX) && num > spec->max) {
        LOG_WARN("set %s fail, %s is larger than cfg_spec max: %u", spec->name, val, spec->max);
        return -1;
    }
...
static in_addr_t cfg_parse_ipv4(const char *val) {
    ...
    if (nr_dot != 3) return INADDR_NONE;
...
static int cfg_spec_set_mac(struct cfg_spec *spec, const char *val) {
    ...
    if (sscanf(val, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &ethaddr.addr_bytes[0], &ethaddr.addr_bytes[1], &ethaddr.addr_bytes[2],
               &ethaddr.addr_bytes[3], &ethaddr.addr_bytes[4], &ethaddr.addr_bytes[5]) != 6) {
...
static int cfg_spec_set_mask(struct cfg_spec *spec, const char *val) {
    ...
    if (is_invalid_mask(mask)) return -1;
...
static int cfg_spec_set(struct cfg_spec *spec, const char *val, int init) {
    if ((spec->flags & CFG_FLAG_RDONLY) && init == 0) return -1;
...
static int cfg_spec_get_ipv6(struct cfg_spec *spec, char *val) {
    ...
    if (dev_ip->prefixlen) tpa_snprintf(val + len, VAL_SIZE - len, "/%d", dev_ip->prefixlen);
...
static int cfg_spec_get_mac(struct cfg_spec *spec, char *val) {
    ...
    tpa_snprintf(val, VAL_SIZE, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", ethaddr->addr_bytes[0], ethaddr->addr_bytes[1], ethaddr->addr_bytes[2],
                 ethaddr->addr_bytes[3], ethaddr->addr_bytes[4], ethaddr->addr_bytes[5]);

    return 0;
}

static int cfg_spec_get_val(struct cfg_spec *spec, char *val) {
    if (spec->get) return spec->get(spec, val);
...
static void usage(struct shell_cmd_info *cmd) {
    shell_append_reply(cmd->reply, "usage: cfg cmd [opts]\n"
...
static int cfg_set(struct shell_cmd_info *cmd) {
    ...
    if (spec == NULL) {
        shell_append_reply(cmd->reply, "error: invalid option name: %s\n", opt_name);
        return -1;
    }
...
int cfg_section_parse(const char *section_name) {
    ...
    while (opt != NULL) {
        ...
        if (spec) ret |= cfg_spec_set(spec, opt->val, 1);
...
static int cfg_spec_register_one(struct cfg_spec *spec) {
    ...
    if (cfg_spec_find(spec->name)) {
        LOG_WARN("failed to register cfg spec %s: already registered", spec->name);
        return -1;
    }

    if (spec->type <= CFG_TYPE_MIN || spec->type >= CFG_TYPE_MAX) {
        LOG_WARN("failed to register cfg spec %s: invaild type: %d", spec->name, spec->type);
        return -1;
    }
...
void cfg_dump_unknown_opts(void) {
    ...
    CFG_SECTION_FOREACH(sect) {
        for (opt = sect->opts; opt != NULL; opt = opt->next) {
            ...
            if (cfg_spec_find(name) == NULL) LOG_WARN("unknown opt: %s.%s", sect->name, opt->name);
        }
    }
}
