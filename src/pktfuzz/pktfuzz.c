...
static const struct fuzzer *fuzzer_find(const char *name) {
    ...
    for (i = 0; i < ARRAY_SIZE(fuzzers); i++) {
        if (strcmp(fuzzers[i]->name, name) == 0) return fuzzers[i];
    }
...
void fuzz(struct port_txq *txq) {
    ...
    for (i = 0; i < ARRAY_SIZE(fuzzers); i++) {
        if (txq->nr_pkt) fuzzers[i]->fuzz(txq);
    }
}
...
void fuzz_run(struct port_txq *txq) {
    ...
    for (i = 0; i < ARRAY_SIZE(fuzzers); i++) {
        if (fuzzers[i]->run) fuzzers[i]->run(txq);
    }
}
...
static int cmd_pktfuzz(struct shell_cmd_info *cmd) {
    ...
    if (cmd->argc < 1) goto out;

    if (strcmp(cmd->argv[0], "help") == 0) return cmd_fuzz_help(&opts);

    if (!pktfuzz_enabled) {
        shell_append_reply(cmd->reply, "erorr: pktfuzz is not enabled.\n"
                                       "\n"
    ...
    if (strcmp(cmd->argv[0], "egress") == 0) {
        ...
        shell_append_reply(cmd->reply, "error: pktfuzz doesn't support ingress fuzz yet\n");
        return -1;
...
void pktfuzz_update_csum_offload(struct packet *pkt) {
    ...
    if (m->pkt_len >= 1500) m->ol_flags |= PKT_TX_TCP_SEG;
