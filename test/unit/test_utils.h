...
#define WHILE_NOT_TIME_UP() for (uint64_t start_tsc = rte_rdtsc(); rte_rdtsc() - start_tsc < ut_test_opts.duration * rte_get_tsc_hz();)

extern uint64_t on_interval_last;
#define ON_INTERVAL(us) if (TSC_TO_US(rte_rdtsc() - on_interval_last) >= (us) && (on_interval_last = rte_rdtsc()))
...
#define ut_free_mbuf_count()                                                                                                               \
    (rte_mempool_ops_get_count(packet_pool_get_mempool(generic_pkt_pool)) + mbuf_cache_count(packet_pool_get_mempool(generic_pkt_pool)))
#define ut_total_mbuf_count() (packet_pool_get_mempool(generic_pkt_pool)->populated_size)

#define ut_assert_mbuf_count()                                                                                                             \
    do {                                                                                                                                   \
...
static inline int ut_time_not_changed(struct timer *timer, struct timer_snapshot *snapshot) {
    return snapshot->slot_idx == timer->slot_idx && snapshot->active == timer->active && snapshot->closed == timer->closed;
}
