...
static void print_entries(int nr_entry) {
    ...
    for (i = 0; i < nr_entry; i++) {
        ...
        if (!(list_all || valid)) continue;

        if (!valid) strcat(entry->path, "[deleted]");

        printf("%-42s %-8lu %-6u %-28s %-6d %s\n", entry->path, entry->off, entry->size, str_time(entry->time), entry->sid, entry->name);
    }
}
...
static void sock_trace_list(struct archive_map *map) {
    ...
    for (i = 0; i < NR_MAP_ENTRY; i++) {
        ...
        if (is_empty_entry(entry)) continue;
...
int main(int argc, char *argv[]) {
    if (argc > 1 && strcmp(argv[1], "-a") == 0) list_all = 1;
