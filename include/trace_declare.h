...
#define TSOCK_TRACE_FOREACH(trace) for (uint64_t off = 0; (trace = tsock_trace_at(off)) && trace->size > 0; off += trace->size)

static inline struct tsock_trace *tsock_trace_at(uint64_t off) {
    if (off >= tsock_trace_ctrl.size) return NULL;
...
static inline char *tsock_trace_name(struct tsock_trace *trace, const char *mark, char *name, size_t size) {
    ...
    tpa_snprintf(name, size, "%s:%hu -> %s:%hu %s", local_ip, ntohs(trace->local_port), remote_ip, ntohs(trace->remote_port), mark);
...
#define TSOCK_TRACE_PROLOG(tsock, nr_record)                                                                                               \
    do {                                                                                                                                   \
        struct tsock_trace *trace = tsock->trace;                                                                                          \
        if (!trace || !tsock_trace_is_enable(tsock)) return;                                                                               \
        if (trace_cfg.no_wrap && trace->off + (nr_record) > trace->mask) return;                                                           \
    } while (0)
...
static inline void tsock_trace_base_init(struct tcp_sock *tsock) {
    ...
    archive_map_add(tsock_trace_ctrl.curr_trace_map, (char *)trace - (char *)tsock_trace_ctrl.file, trace->init_time, trace->size,
                    trace->sid, name, tpa_cfg.sock_trace_file);
}
...
#define TYPE_RECORD(_type, ...)                                                                                                            \
    record = &tsock->trace->records[(tsock->trace->off++) & tsock->trace->mask];                                                           \
    record->type = _type;                                                                                                                  \
    __VA_ARGS__
#define DATA_RECORD(...)                                                                                                                   \
    record = &tsock->trace->records[(tsock->trace->off++) & tsock->trace->mask];                                                           \
...
static inline int trace_record_fetch(struct trace_ctx *ctx, struct trace_record *records, uint64_t count) {
    ...
    if (ctx->iter + count > ctx->trace->off) return -1;
...
#define DECLARE_TRACE(name, nr_record, args, unpack_records, parser)                                                                       \
    static inline int trace_dump_##name(struct trace_ctx *ctx) {                                                                           \
        ...
        args;                                                                                                                              \
                                                                                                                                           \
        if (trace_record_fetch(ctx, records, nr_record) < 0) return -1;                                                                    \
                                                                                                                                           \
        unpack_records;                                                                                                                    \
        parser;                                                                                                                            \
        return 0;                                                                                                                          \
