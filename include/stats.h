...
static inline int stats_code_check(int stats) {
    if ((uint32_t)stats >= STATS_MAX) return ERR_INVALID_STATS;

    return stats;
}

#define STATS_ADD(base, stats, n)                                                                                                          \
    do {                                                                                                                                   \
...
#define WORKER_TSOCK_STATS_INC(worker, tsock, stats)                                                                                       \
    do {                                                                                                                                   \
        WORKER_STATS_INC(worker, stats);                                                                                                   \
        if (tsock) TSOCK_STATS_INC(tsock, stats);                                                                                          \
    } while (0)

#define WORKER_TSOCK_STATS_ADD(worker, tsock, stats, n)                                                                                    \
    do {                                                                                                                                   \
        WORKER_STATS_ADD(worker, stats, n);                                                                                                \
        if (tsock) TSOCK_STATS_ADD(tsock, stats, n);                                                                                       \
    } while (0)
 ...
 */
#define STATS_ADD_ATOMIC(base, stats, n)                                                                                                   \
    do {                                                                                                                                   \
...
#define WORKER_TSOCK_STATS_INC_ATOMIC(worker, tsock, stats)                                                                                \
    do {                                                                                                                                   \
        WORKER_STATS_INC_ATOMIC(worker, stats);                                                                                            \
        if (tsock) TSOCK_STATS_INC_ATOMIC(tsock, stats);                                                                                   \
    } while (0)

#define WORKER_TSOCK_STATS_ADD_ATOMIC(worker, tsock, stats, n)                                                                             \
    do {                                                                                                                                   \
        WORKER_STATS_ADD_ATOMIC(worker, stats, n);                                                                                         \
        if (tsock) TSOCK_STATS_ADD_ATOMIC(tsock, stats, n);                                                                                \
    } while (0)
...
static inline void vstats_add(struct vstats *vstats, uint64_t val) {
    ...
    if (vstats->max < val) vstats->max = val;
