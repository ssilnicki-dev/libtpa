...
static char *parser_dump(const char *parser_name, const char *mem_file_path) {
    ...
    if (!getenv("TPA_USE_SYSTEM_PARSER")) {
        ...
        if (mem_file == NULL) return NULL;

        if (do_dump_parser(mem_file, parser_name) == 0) return dump_path;
    }
...
static int show_mem_file_info(const char *path) {
    ...
    printf("%s:\n"
           ...
           "  parser_size %lu\n"
           "  name        %s\n",
           path, mem_file->hdr->data_offset, mem_file_data_size(mem_file), mem_file->hdr->parser_offset, mem_file_parser_size(mem_file),
           mem_file->hdr->name);
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "-i") == 0) return show_mem_file_info(argv[2]);
